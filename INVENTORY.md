...
`acquire`* · `adjudicate` · `advise` · `beat`* · `box` · `catch`* · `evolve`* · `faint`* · `find` · `give`* · `graveyard` · `heartscale`* · `hold`* · `learn` · `level`* · `log` · `matrix` · `milestones` · `new`* · `next` · `nickname`* · `party`* · `plan` · `rank` · `release`* · `roll` · `routes` · `scout` · `spend`* · `split` · `status` · `take`* · `teach`* · `undo`* · `unhold`* · `use`* · `where`
...
- `run.js`: `adviseCatches`, `adviseUpgrades`, `apply`, `applyAll`, `boxMatrix`, `capAt`, `createRun`, `encounterRules`, `encountersOn`, `fieldItems`, `fightTier`, `findMon`, `isExcludedVariant`, `learnable`, `levelCap`, `milestones`, `partySpecs`, `planNext`, `rankParties`, `requireLayer`, `rollEncounter`, `split`, `splitPrep`, `summarize`, `undo`, `unusedRoutes`, `upcoming`
