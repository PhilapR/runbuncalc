...
`acquire` · `beat` · `catch` · `evolve` · `faint` · `give` · `heartScale` · `hold` · `levelUp` · `nickname` · `party` · `release` · `spend` · `take` · `teach` · `unhold` · `use`

## CLI subcommands (`play.js`)
`acquire`* · `adjudicate` · `advise` · `beat`* · `box` · `catch`* · `evolve`* · `faint`* · `find` · `give`* · `graveyard` · `heartscale`* · `hold`* · `learn` · `level`* · `log` · `matrix` · `milestones` · `new`* · `next` · `nickname`* · `party`* · `plan` · `playbook` · `rank` · `release`* · `roll` · `routes` · `scout` · `spend`* · `split` · `status` · `take`* · `teach`* · `undo`* · `unhold`* · `use`* · `where`
