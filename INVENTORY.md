...
`acquire` · `beat` · `catch` · `evolve` · `faint` · `give` · `heartScale` · `hold` · `levelUp` · `nickname` · `party` · `release` · `spend` · `take` · `teach` · `unhold`

## CLI subcommands (`play.js`)
`acquire`* · `adjudicate` · `advise` · `beat`* · `box` · `catch`* · `evolve`* · `faint`* · `find` · `give`* · `graveyard` · `heartscale`* · `hold`* · `learn` · `level`* · `log` · `matrix` · `milestones` · `new`* · `next` · `nickname`* · `party`* · `plan` · `rank` · `release`* · `roll` · `routes` · `scout` · `spend`* · `split` · `status` · `take`* · `teach`* · `undo`* · `unhold`* · `where`
...
- **ball-economy** (2026-08-12): Wild fights throw plain Poke Balls with no inventory or cost. Should balls be bag items (bought, finite, better tiers with real multipliers), and does Run & Bun change any ball bonuses? _Settled by: operator ruling._
