...
`acquire`* · `advise` · `beat`* · `box` · `catch`* · `evolve`* · `faint`* · `find` · `give`* · `graveyard` · `heartscale`* · `hold`* · `learn` · `level`* · `log` · `matrix` · `milestones` · `new`* · `next` · `nickname`* · `party`* · `plan` · `rank` · `release`* · `routes` · `scout` · `split` · `status` · `take`* · `teach`* · `undo`* · `unhold`* · `where`
...
- `run.js`: `adviseCatches`, `adviseUpgrades`, `apply`, `applyAll`, `boxMatrix`, `capAt`, `createRun`, `encounterRules`, `encountersOn`, `fightTier`, `findMon`, `isExcludedVariant`, `learnable`, `levelCap`, `milestones`, `partySpecs`, `planNext`, `rankParties`, `requireLayer`, `rollEncounter`, `split`, `splitPrep`, `summarize`, `undo`, `unusedRoutes`, `upcoming`
