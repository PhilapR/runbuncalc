...
`acquire`* · `advise` · `beat`* · `box` · `catch`* · `evolve`* · `faint`* · `find` · `give`* · `graveyard` · `heartscale`* · `learn` · `level`* · `log` · `matrix` · `milestones` · `new`* · `next` · `nickname`* · `party`* · `plan` · `release`* · `routes` · `split` · `status` · `take`* · `teach`* · `undo`* · `where`
...
- `run.js`: `adviseUpgrades`, `apply`, `applyAll`, `boxMatrix`, `capAt`, `createRun`, `encounterRules`, `encountersOn`, `fightTier`, `findMon`, `isExcludedVariant`, `learnable`, `levelCap`, `milestones`, `partySpecs`, `planNext`, `requireLayer`, `split`, `splitPrep`, `summarize`, `undo`, `unusedRoutes`, `upcoming`
