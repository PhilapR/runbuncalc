...
test('every battery scenario reads a document the repository tracks', () => {
	...
	// Every manifest but dose.json: its eight documents were never in the
	// 2026-09-14 archive, so they cannot be banked as they were measured —
	// only regenerated by scripts/scale-dose.js under today's advisor.
	const untracked = ['battery.json', 'heldout.json', 'heldout2.json',
		'choice.json', 'counterfactuals.json', 'strategies.json'].flatMap(file => JSON.parse(
