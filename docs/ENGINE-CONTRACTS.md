# Engine transition contracts

These are the per-mechanic contracts for `ai/` state transitions. Each entry
records a boundary that a past change broke or almost broke. Read this file
before you edit a move, an ability, an item effect, or a transition in
`ai/src/`. `AGENTS.md` holds the always-applicable rules; this file holds the
case law.
