...
| Price the voluntary switch — lead from the re-pick tapes | the lost-race resist check reads the threat line's move, which is the hardest hit against the active body; price each candidate's entry against the foe's best move into IT, as forced replacements already are | new arm; the ten vetoed heldout2/heldout1 scenarios were read to find it, so they are in-sample for it |
