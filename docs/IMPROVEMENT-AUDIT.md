...
| Catch planner v2 — the early road priced for Brawly 2026-09-18 (ab9ad9d; Tyrogue fix 9d0f602) | per route, the method whose one random encounter is worth most at the gym ahead, from a key measured across 19 boxes instead of six fights per box | Key (scenarios/catch-values/leader-brawly.json, 30 catches, control 30/380): Qwilfish (Slateport) 80, Tyrogue as Hitmontop 79, Combee 75, as Hitmonlee 48, as Hitmonchan 17 — the dossier's named answer is the worst branch. Tyrogue branches by Atk vs Def at 20 (the importer had dropped EVO_LEVEL_ATK_*; fixed): Hitmonchan 44.3%, Hitmonlee 44.3%, Hitmontop 11.5%, so a Tyrogue is worth +2.1 per 100, not +12.9. One encounter per route caps any route near +1.3 per 100 in expectation (Slateport fishing, Qwilfish 10%). Open: gains are against these boxes' own catch per route; routes priced independently; Combee's gender rule unverified; not yet validated on boxes outside the key |
