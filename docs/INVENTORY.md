...
`acquire` · `beat` · `catch` · `consume` · `evolve` · `faint` · `give` · `heartScale` · `hold` · `identify` · `levelUp` · `nickname` · `party` · `release` · `skip` · `spend` · `take` · `teach` · `unhold` · `use`
...
- `run.js`: `adviseCatches`, `adviseUpgrades`, `affordableFromBag`, `affordableOptions`, `answerTable`, `apply`, `applyAll`, `boxMatrix`, `capAt`, `createRun`, `dupeKey`, `eliteFourCounterpart`, `encounterRules`, `encountersOn`, `evolutionReadiness`, `fieldItems`, `fightPlaybook`, `fightTier`, `findMon`, `isExcludedVariant`, `learnable`, `levelCap`, `megaFormOf`, `megaRingHeld`, `milestones`, `pairingLethal`, `partySpecs`, `planNext`, `playedOrder`, `preFightOpportunities`, `rankParties`, `requireLayer`, `rollEncounter`, `rollIdentity`, `rollIvs`, `safetyPath`, `shopItems`, `spendFromBag`, `split`, `splitPrep`, `stoneInBag`, `summarize`, `trainerIndexOf`, `undo`, `unusedRoutes`, `upcoming`
...
Trainer fights in the run map: 366

Oracle datasets:
- `ability-reconciliation.json`: 0 entries
...
- `item-locations.json`: 296 entries
...
- **a-voluntary-switch-is-priced** (2026-09-18): A voluntary switch is priced the way a forced replacement is: the candidate is seated and charged the hit it concedes on the way in, and the panel shows that race on every switch button. The automated policy's lost-race switch goes only to a body that wins that race; with no winner it stays and attacks. --switch-priced=0 (and driver.setSwitchPricing(false)) restore the old type-multiplier switch for arms that measure against it.
- **the-battery-re-picks-like-a-live-run** (2026-09-18): A battery scenario fields the ranker's six against its own trainer, lead first, as a live run does when it presses Rank before a fight. --repick-party=0 plays the banked party verbatim. Receipts record the defaults they ran under, and battery-tape.js replays a receipt without that record under the pre-adoption defaults.
- **the-battery-picks-the-six-by-play** (2026-09-18): A re-picking battery scenario fields, of the ranker's first six sixes, the one that wins most of six selection fights played by the real policy on seeds 100001-100006, disjoint from every evaluation seed; a tie keeps the ranker's order. --pick-by-play=0 fields the ranker's first six. The receipt records every tally; battery-tape.js replays a receipt without the record as --pick-by-play=0. The live run still fields the ranker's first six.
- **the-set-score-prices-shared-weakness** (2026-09-18): rankParties charges a six EXPOSURE_WEIGHT (0.5) per member an enemy one-shots from the slower side, summed over enemies, in the live ranker and the battery alike. exposureWeight: 0 and --set-exposure=0 restore the ranker as it stood. Receipts record the weight; battery-tape.js replays a receipt without the record at 0.
- **races-read-the-engine-speed** (2026-09-19): The race, the threat line and the switch prices read each Pokemon's effective Speed from the engine (ai.getEffectivePokemonSpeed). --real-speed=0 (driver.setRealSpeed(false)) restores the old constant zero for arms that measure against it; battery-tape.js replays a receipt without the record at 0.
- **a-double-is-searched-jointly** (2026-09-19): A double is played with our two actives searched as ONE choice (driver.setDoublesJoint, --doubles-joint=1), four rollouts a pair (--doubles-search=4), and the party is taught its doubles tools before the fight (--doubles-prep=1: Fake Out, Wide Guard, Icy Wind, Protect, Helping Hand, Tailwind, at most two bodies, never over a priority move). --doubles-search=0 restores the engine's own hand.
- **a-tm-is-a-one-time-item** (2026-09-20): A TM is a ONE-TIME item: a move whose only free route is the teachable pool requires that TM in the bag and spends it, unless the TM is an HM (reusable) or one of the ten the Lilycove Department Store re-sells. A move a tutor teaches stays free, because a tutor is a service. The learnable surfaces name the TM a move costs and whether the bag holds one, so no surface offers what the run cannot pay for.
...
- **losers-are-ranked-by-work** (2026-09-21): When every forced replacement loses its race, the automated hand sends the one that removes the most of the foe before it falls (acts before death over turns to kill, from the priced race the panel already prints on each switch button). A replacement that wins is never displaced and winners keep their order. --loser-work=0 restores armed, resistance, health.
...
- **gen8-gate-order-infatuation-before-paralysis** (2026-09-22): SUPERSEDED 2026-09-22 by rom-gate-order-paralysis-before-infatuation. Was: infatuation checked before paralysis (the Gen 8 order).
- **rom-gate-order-paralysis-before-infatuation** (2026-09-22): A Pokemon both paralysed and infatuated is checked for paralysis BEFORE infatuation, as the ROM does. The action gates run recharge, sleep/freeze, Truant, flinch, confusion, paralysis, infatuation.
...
- **do-gifts-consume-an-area-encounter** (2026-09-21): The story gifts are now claimed by the run (--gifts=1: the Lavaridge egg at 571, Castform at 701, Kubfu at 1125; scripts/headless-run.js claimGifts). A gift is a scripted event, not an encounter-table roll, so the run does NOT count it against its area's wild encounter, and does apply the dupes clause. Is that the rule you play by? The alternative — a gift spends the encounter for Lavaridge, Route 119 and Mossdeep — costs three wild catches and is what some nuzlocke rulesets require. Also unwired and waiting on the same answer: the three trades (they consume a body we own), the fossils, and the seven roaming legendaries (undated, gated on the Sootopolis story event, so late and cheap to leave). _Settled by: operator ruling._
- **what-rule-replaces-a-knocked-out-foe** (2026-09-22): When one of the enemy trainer's Pokemon faints, which one do they send out? We apply the FIRST legal replacement the engine enumerates (lib/battle-driver.js settleAiSide), and the engine's own docstring says that choice is meant to be made by the policy layer. Measured over 1,328 logged fights: 1,240 send-outs matched the fight data's party order and 88 did not, all singles — so it is the battle state's enumeration order, and never a decision. The hack may score replacements (our ai package has evaluateForcedSwitchActions, unused here), or it may send the next in party order; nothing in this repository establishes which, and it was not guessed. Needs the decomp, the hack's AI source, or emulator observation. Raised by Codex 2026-09-22; ledger finding enemy-post-ko-replacement-is-enumeration-order carries the evidence and the bar for changing it. _Settled by: operator ruling._
...
- `AGENTS.md` — 12701edc 2026-09-22
- `README.md` — 1b6ba2a8 2026-09-22
- `docs/AI_DATA_MODEL.md` — 784dc619 2026-09-22
- `docs/CONSTANTS-AUDIT.md` — 784dc619 2026-09-22
- `docs/DATA-ACCESS.md` — c272bcde 2026-08-25
- `docs/DEPLOY.md` — 98b4a50d 2026-08-18
- `docs/ECOSYSTEM_BRIDGE.md` — 5a255c09 2026-08-25
- `docs/ENCOUNTER-ARCHITECTURE.md` — 29ffa9fc 2026-09-20
- `docs/ENGINE-CONTRACTS.md` — c4daad83 2026-08-29
- `docs/ENGINE-HOMES.md` — a464f32c 2026-09-22
- `docs/EVALUATION.md` — 56f1c219 2026-09-22
- `docs/FORK_MAP.md` — 202de902 2026-09-22
- `docs/GEN9_AUDIT.md` — 98b4a50d 2026-08-18
- `docs/HOW-WE-WIN.md` — 4ecbdc3d 2026-09-21
- `docs/IMPROVEMENT-AUDIT.md` — 6034c5c0 2026-09-20
- `docs/LEADER-KEYS.md` — 44ad51e7 2026-09-21
- `docs/MODELLING-GAPS.md` — 56f1c219 2026-09-22
- `docs/PERFORMANCE.md` — f6708e50 2026-09-22
- `docs/PLAN.md` — e342a33d 2026-09-22
- `docs/RUN-CONTROL.md` — 723d4175 2026-09-22
- `docs/RUNBUN_REBUILD_MODEL.md` — 56f1c219 2026-09-22
- `docs/RUNBUN_UI_DESIGN.md` — 56f1c219 2026-09-22
- `docs/RUN_API.md` — 6234521d 2026-08-28
- `docs/SDLC.md` — e342a33d 2026-09-22
- `docs/STRATEGY.md` — c272bcde 2026-08-25
- `docs/VALIDATION.md` — 56f1c219 2026-09-22
- `docs/WORKFLOW.md` — 6fc3aebb 2026-09-23
- `docs/WORST-CASE-PLANNING.md` — a7f3d1e9 2026-09-22
- `ui-playthrough-out.ARCHIVED.md` — untracked

## Test files
`tests/ability_reconciliation.test.js` · `tests/adjudication_cost.test.js` · `tests/ai-probes.test.js` · `tests/ask.test.js` · `tests/attempt_store.test.js` · `tests/audit_run.test.js` · `tests/availability-estimate.test.js` · `tests/battery_arms.test.js` · `tests/battery_receipts.test.js` · `tests/battle-driver.test.js` · `tests/battle_contribution.test.js` · `tests/battle_view.test.js` · `tests/browser_calc_load.test.js` · `tests/browser_entrypoint.test.js` · `tests/browser_planner.test.js` · `tests/browser_run.test.js` · `tests/browser_run_fight.test.js` · `tests/browser_run_panel.test.js` · `tests/browser_run_setup.test.js` · `tests/browser_skip_guard.test.js` · `tests/catch_values.test.js` · `tests/chronicle.test.js` · `tests/doubles.test.js` · `tests/encounter_dating.test.js` · `tests/engine_mcp.test.js` · `tests/evolution_branches.test.js` · `tests/falsify.test.js` · `tests/fast_tier.test.js` · `tests/fidelity_openings.test.js` · `tests/fight_dossiers.test.js` · `tests/fight_fields.test.js` · `tests/fight_log.test.js` · `tests/frontier_set.test.js` · `tests/game-runtime-adapter.test.js` · `tests/headless_run.test.js` · `tests/how_it_won.test.js` · `tests/import_oracle.test.js` · `tests/inventory.test.js` · `tests/item_facts.test.js` · `tests/item_locations.test.js` · `tests/item_planning.test.js` · `tests/learnset_doc.test.js` · `tests/ledger.test.js` · `tests/manifest_provenance.test.js` · `tests/mega_forms.test.js` · `tests/nuz_compare.test.js` · `tests/order_scales.test.js` · `tests/pivot_turn.test.js` · `tests/planner.test.js` · `tests/planning_review.test.js` · `tests/play.test.js` · `tests/playthrough_panel.test.js` · `tests/playthrough_policy.test.js` · `tests/pokemon-bridge.test.js` · `tests/pokemon-provider-client.test.js` · `tests/provenance.test.js` · `tests/provider-data.test.js` · `tests/recheck_findings.test.js` · `tests/rl-dataset.test.js` · `tests/rom-band.test.js` · `tests/run.test.js` · `tests/run_advise.test.js` · `tests/run_api.test.js` · `tests/run_batch.test.js` · `tests/run_control.test.js` · `tests/run_history.test.js` · `tests/run_rank.test.js` · `tests/runbun_mechanics.test.js` · `tests/runbun_oracle.test.js` · `tests/runbun_policy.test.js` · `tests/runbun_sets.test.js` · `tests/runbun_species.test.js` · `tests/scale_consistency.test.js` · `tests/search_policy.test.js` · `tests/semi_invulnerable.test.js` · `tests/server.smoke.test.js` · `tests/sets_to_battle_state.test.js` · `tests/simulate.test.js` · `tests/slots.test.js` · `tests/team.test.js` · `tests/tm_sourcing.test.js` · `tests/tracker_order.test.js` · `tests/trainer_orders.test.js` · `tests/ui_style.test.js` · `tests/wall_report.test.js` · `tests/watch.test.js` · `tests/worker-runtime.test.js` · `tests/worker.test.js`

## Prior art elsewhere (from ECOSYSTEM.json)

### PhilapR/pokemon-mono
The operator's primary monorepo — four earlier repos subtree-imported (see its RECEIPTS-MAP.md). Home of the ROM ground truth (pykemon) and the evidence every engine is graded against; home of any aspect where its implementation grades best against the ROM (enemy AI move scoring today). Which platform is home for each aspect of the game is decided by evidence in docs/ENGINE-HOMES.md (approved 2026-09-22), not by repository.
