...
| **Enemy AI move choice** | **rab `agent/complete-ai-scoring.ts`**: fitted to 52 ROM probes, held-out top-1 0.972 | runbuncalc `ai/src/scoring.ts`, frozen since 08-12, SETUP still transcribed, **no probe replay**; rlm partial | ROM score arrays, 1,040/1,040 rows | **replay the probes against runbuncalc, then import rab's corrections.** The app's turn advice uses the weaker copy today |
