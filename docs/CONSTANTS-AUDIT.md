...
- Fix: move the paralysis check between confusion and infatuation (the emerald-lineage order, better supported for this hack); confusion-before-paralysis is the load-bearing part. Flinch-before-sleep is harmless (both deterministic). (Superseded 2026-09-22: it was not harmless — a flinched sleeper did not spend its sleep turn. Sleep and freeze now gate before Truant and flinch, `b193e3f`. The paralysis/infatuation order chosen here was replaced by the Gen 8 order on 2026-09-22 (`4ecdf5e`), then RESTORED the same day because the ROM measured it: over 300 turns a paralysis message never followed a love message (pokemon-mono P4). D12's pick was right.)
