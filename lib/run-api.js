...
function asRefusal(error) {
	if (/^[A-Z][a-z].*(does not|cannot|is not|has no)|^no |^teach:|^evolve:|^catch:|^identify:|^levelUp:|^party:|^give:|^take:|^beat:|^acquire:|^use:|^faint:|^heartScale:|^hold:|^unhold:|^spend:|^roll:|^battle:|^playbook:|^rank:|^unknown |^a command needs|^nothing |^the party is empty|^a party holds/.test(error.message || '')) {
		error.statusCode = 400;
...
const api = {
	...
	status(payload) {
		...
		return guarded(() => {
			...
			return {
				...
				// The open marts' stock, each row saying whether this run already
				// bought it — stones fund the advisor's evolve rows, and a client
				// that cannot see the shop cannot buy them.
				shop: runtime.shopItems(state),
				...
				upcoming: runtime.upcoming(state, count).map(fight => {
					const profile = require('../profiles').getProfile(state.profileId);
					return {
						...
						tier: runtime.fightTier(profile, fight.trainer),
						// The profile's declared exceptions to required-in-place —
						// Camper Gavi is MEANT to be passed and taken after the
						// museum grunts — so the client can offer the skip, and say
						// when one is already owed.
						skippable: (profile.delayableFights || []).includes(fight.trainer) ||
							(profile.optionalFights || []).includes(fight.trainer) ||
							// Any double battle: doubles play refuses (real
							// doubles, not modeled yet), so the skip is the way
							// past and the client must be able to offer it.
							!!fight.isDouble,
						skipped: (state.skipped || []).includes(fight.order),
	...
	/** What the selected Pokemon evolves into, and whether it is ready. */
	evolutions(payload) {
		const state = requireRun(payload);
		return guarded(() => runtime.evolutionReadiness(state, (payload || {}).id));
	},
	...
	plan(payload) {
		...
		return guarded(() => {
			...
			return {
				...
				// sash or pinch-berry Reversal/Flail/Endeavor. The fair-dice
				// sampler is known blind to that scaling, so the verdict has to
				// carry the warning, and the verdict is built from this reply.
				thresholdThreats: plan.thresholdThreats,
				// The number a player counts in: the Nth trainer on the map, not
				// the cumulative-Pokemon order the data is keyed by.
				fightNumber: plan.fightNumber,
				// The kit's declared tech, from the fight-dossier oracle: what
				// this plan must respect before any move choice matters.
				dossierTech: plan.dossierTech,
			};
		});
	},
	...
	rank(payload) {
		...
		// The way back to the whole box. A large box is cut to the members that
		// answer somebody, which is right for ranking and wrong for the question
		// "what could I sacrifice here" — so the caller can ask for all of it and
		// wear the cost, which the library still bounds.
		const exhaustive = (payload || {}).exhaustive;
		if (exhaustive !== undefined) {
			if (typeof exhaustive !== 'boolean') {
				throw refusal('exhaustive must be true or false', 'InvalidRunCommand');
			}
	...
	playbook(payload) {
		...
		// The assignment search is the expensive half and the accurate half.
		// `rollouts` alone never reached it — it sized the final line while the
		// search stayed pinned at four — so the knob that decides the plan is
		// its own knob, bounded like the rest.
		const variantRollouts = (payload || {}).variantRollouts;
		if (variantRollouts !== undefined &&
			(!Number.isInteger(variantRollouts) || variantRollouts < 1 || variantRollouts > 24)) {
			throw refusal('variantRollouts must be an integer from 1 to 24',
				'InvalidRunCommand');
		}
		const options = {};
		if (rollouts) options.rollouts = rollouts;
		if (variantRollouts) options.variantRollouts = variantRollouts;
	...
	/** Pre-fight catch odds for a rolled species: decide battle/keep/flee
	 * with the same numbers the fight's ball buttons will wear. */
	catchOdds(payload) {
		const state = requireRun(payload);
		const species = (payload || {}).species;
		if (!species || typeof species !== 'string') {
			throw refusal('species is required — send what /run/encounter rolled', 'InvalidRunCommand');
		}
		return guarded(() => ({odds: battleDriver.catchOddsAtFullHp(state, species)}));
	},

	/** What, if anything, makes the next fight survivable — and its price. */
	safety(payload) {
		const state = requireRun(payload);
		return guarded(() => runtime.safetyPath(state, (payload || {}).trainer));
	},
	...
	identity(payload) {
		...
		// Everything this endpoint serves is a scripted gift — starter, gift,
		// static, trade — and the gift class carries Run & Bun's three
		// guaranteed perfect IVs (operator ruling on the starter, 2026-08-28;
		// extended to the endpoint's whole class because it cannot tell them
		// apart and they share the mechanic in the modern games the hack
		// follows — correct the class list if the hack splits them).
		return guarded(() => ({identity:
			runtime.rollIdentity(payload.species.trim(), undefined, {perfectIvs: 3})}));
	},
...
const ROUTES = {
	...
	'/run/safety': api.safety,
	'/run/catch-odds': api.catchOdds,
