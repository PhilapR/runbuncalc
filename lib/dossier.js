...
/**
 * A stat at a level, the Gen 3+ way: no EVs (the run records none), the
 * nature's tenth up or down. Used only to decide a stat-conditioned branch.
 */
function statAt(species, stat, level, iv, nature) {
	const calc = require('../calc');
	const gen = calc.Generations.get(8);
	const found = gen.species.get(calc.toID(species));
	if (!found) throw new Error('no species data for ' + species);
	const raw = Math.floor((2 * found.baseStats[stat] + iv) * level / 100) + 5;
	const mood = nature ? gen.natures.get(calc.toID(nature)) : null;
	const factor = mood && mood.plus !== mood.minus ?
		(mood.plus === stat ? 1.1 : mood.minus === stat ? 0.9 : 1) : 1;
	return Math.floor(raw * factor);
}

/**
 * Whether a path's `requires` holds for this Pokemon. Tyrogue branches by its
 * own Attack against Defense when it evolves (EVO_LEVEL_ATK_GT_DEF and kin,
 * which the importer used to fold into plain 'level'). Gender is not
 * modelled by the run, so a gender requirement is not judged here: it
 * answers true, as every consumer assumed before `requires` existed.
 */
function meetsRequirement(step, species, level, ivs, nature) {
	const rule = step.requires;
	if (!rule || rule === 'female' || rule === 'male') return true;
	// The game judges the branch on the level-up that reaches the evolution
	// level (Tyrogue: 20), so the stats are read there, not at a later level.
	const at = step.level || level;
	const atk = statAt(species, 'atk', at, (ivs || {}).atk || 0, nature);
	const def = statAt(species, 'def', at, (ivs || {}).def || 0, nature);
	if (rule === 'atk>def') return atk > def;
	if (rule === 'atk<def') return atk < def;
	if (rule === 'atk=def') return atk === def;
	throw new Error('unknown evolution requirement ' + rule);
}

/**
 * evolveTo for one Pokemon rather than a species: a stat-conditioned branch
 * goes the way this Pokemon's IVs and nature send it. evolveTo, which has no
 * Pokemon, still takes the first path.
 */
function evolveMon(mon, level, stones) {
	let current = mon.species;
	for (let hops = 0; hops < 3; hops++) {
		const from = current;
		const step = (evolutions[from] || []).find(p =>
			((p.method === 'level' && p.level <= level) ||
				(p.method === 'item' && stones && stones.has(p.item))) &&
			meetsRequirement(p, from, level, mon.ivs, mon.nature));
		if (!step) break;
		current = step.into;
	}
	return current;
...
/**
 * The chance of each form a fresh catch of `species` becomes by `level`,
 * over the run's roll: every IV 0-31 alike, every nature alike (rollIdentity).
 * Only the stats a requirement reads can matter, so only Attack and Defense
 * are enumerated: 32 x 32 x 25.
 */
const branchMemo = new Map();

function branchOdds(species, level) {
	const key = species + '@' + level;
	if (!branchMemo.has(key)) branchMemo.set(key, countBranches(species, level));
	return Object.assign({}, branchMemo.get(key));
}

function countBranches(species, level) {
	const natures = [...require('./team').NATURES];
	const counts = {};
	let total = 0;
	for (let atk = 0; atk < 32; atk++) {
		for (let def = 0; def < 32; def++) {
			for (const nature of natures) {
				const form = evolveMon({species, ivs: {atk, def}, nature}, level);
	...
	const odds = {};
	for (const form of Object.keys(counts).sort()) odds[form] = counts[form] / total;
	return odds;
}
...
module.exports = {
	periodPool, evolveTo, evolveMon, meetsRequirement, branchOdds, statAt, lastFourMoves, capOf, buildSample, stratify,
