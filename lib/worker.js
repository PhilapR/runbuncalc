...
// The ledger's read surface. Filters are an ALLOWLIST of column names bound
// as parameters — the query string never reaches SQL as text. There is
// deliberately no /ledger/query endpoint: arbitrary SQL over a binding is a
// door, and this app's whole posture is that the gate fails closed.
const LEDGER_FILTERS = {
	findings: ['status', 'severity', 'area', 'source', 'verdict', 'falsified'],
	decisions: [],
	open: [],
};
const LEDGER_TABLES = {findings: 'findings', decisions: 'decisions', open: 'open_questions'};

async function ledgerAnswer(url, env) {
	if (!env.LEDGER) {
		return Response.json({
			error: 'The ledger is not bound to this Worker.',
			hint: 'Create the D1 database and add the LEDGER binding — see wrangler.jsonc.',
		}, {status: 503});
	}
	const name = url.pathname.slice('/ledger/'.length).replace(/\/$/, '');
	if (name === 'stats') {
		const rows = await env.LEDGER.prepare(
			'SELECT status, severity, COUNT(*) AS n FROM findings GROUP BY status, severity').all();
		const unproven = await env.LEDGER.prepare(
			"SELECT COUNT(*) AS n FROM findings WHERE status = 'fixed' AND falsified = 0").first();
		return Response.json({counts: rows.results, fixedButNeverFalsified: unproven.n},
			{headers: {'Cache-Control': 'no-store'}});
	}
	const table = LEDGER_TABLES[name];
	if (!table) {
		return Response.json({error: `No ledger view named '${name}'.`,
			views: [...Object.keys(LEDGER_TABLES), 'stats']}, {status: 404});
	}
	const where = [];
	const bindings = [];
	for (const column of LEDGER_FILTERS[name]) {
	...
	const answered = await env.LEDGER.prepare(sql).bind(...bindings).all();
	return Response.json(answered.results, {headers: {'Cache-Control': 'no-store'}});
}
