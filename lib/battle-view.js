...
function viewOf(reply) {
	...
	return {
		...
		// What the foe holds and its ability: trainer sets are public in R&B,
		// and a Focus Sash or Sturdy decides which of our moves can finish it.
		foeItem: vs.foe.active.item || null,
		foeAbility: vs.foe.active.ability || null,
		result: reply.result || '',
		...
		switches: (reply.actions || []).filter(entry => entry.kind === 'switch')
			.map(entry => ({
				...
				// The numbers behind the word. The panel already prints them on
				// every switch button ("you need 3, they need 1"), so a player
				// sees them; the automated hand was handed only the word, and
				// with three losers on the bench could not tell the body that
				// trades a hit from the one that dies having done nothing.
				raceDetail: entry.race && entry.race.turnsToKill !== undefined ? {
					turnsToKill: entry.race.turnsToKill,
					turnsToDie: entry.race.turnsToDie,
					faster: entry.race.faster,
					// Both outcomes priced: how it goes with no crit, with every crit, and the odds between.
					...(entry.race.odds ? {odds: entry.race.odds} : {}),
				} : null,
			})),
