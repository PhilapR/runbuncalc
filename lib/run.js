...
/** What the nurse charges to change a nature. */
const NATURE_SCALES = 3;
// Whether a held item survives being used. lib/item-facts derives it from the
// engine's own consumeItem call sites rather than from a hand list, so the
// answer moves when the engine does.
const isSingleUse = require('./item-facts').isSingleUse;
...
/**
 * The ceiling on `rankParties`' exhaustive enumeration, in combinations.
 *
 * The ranker scores every C(box, 6) six off a cached grid, which is cheap per
 * combination and unbounded in how many there are. Measured at roughly 1.24M
 * combinations a second: a box of 30 is 593,775 and takes 1.6s, a box of 42 is
 * 5.2M, and a box of 76 is 218,618,940 and takes 177 SECONDS with no warning.
 *
 * Five million puts the ceiling at a box of 41 — C(41,6) is 4,496,388 and
 * C(42,6) is 5,245,786 — which is deliberately the knee the measurement found,
 * not a round number. Below it nothing changes and every answer is the same
 * exhaustive one; above it the call refuses instead of disappearing for three
 * minutes. A nuzlocke never approaches this because one-per-route holds the box
 * near ten; a level-caps run that rerolls areas reaches 76 easily.
 *
 * Stated in WORK rather than seconds on purpose — the same reason
 * `tests/adjudication_cost.test.js` counts constructed objects. A second is a
 * property of the machine; a combination is a property of the request.
 */
const RANK_COMBINATION_LIMIT = 5000000;

/**
 * The set score's charge per member an enemy one-shots from the slower side.
 * Adopted 2026-09-18 by operator ruling (DECISIONS: the-set-score-prices-shared-weakness).
 * 0.25 fielded the same six in 75 of heldout3's 80 fights.
 */
const EXPOSURE_WEIGHT = 0.5;

/**
 * The box size at which `rankParties` starts cutting, in combinations.
 *
 * A cut that runs on every box would be a cost with no benefit on the boxes
 * that were never expensive — and worse than no benefit. A seven-member box is
 * C(7,6) = 7 sixes, and the cut reduces it to ONE, which is the right answer
 * and an empty shortlist: the `without the star` and `different lead` rows have
 * nothing left to differ with. Below this line the whole box is enumerated and
 * the answer is exactly what it was before the cut existed.
 *
 * A million combinations is about 800ms at the measured 1.24M/s, which keeps
 * every box up to 32 — C(32,6) is 906,192 — on the untouched path, including
 * the box of 30 the suite has always gated at 5s. C(33,6) is 1,107,568 and
 * starts cutting.
 */
const RANK_PRUNE_ABOVE = 1000000;
 ...
 */
/** The distinct abilities a species can have, in slot order. */
function abilityList(species) {
	const calc = require('../calc');
	const found = calc.Generations.get(8).species.get(calc.toID(species));
	if (!found || !found.abilities) return [];
	return [...new Set(Object.values(found.abilities).filter(Boolean))];
}

function rollIdentity(species, random, opts) {
	const draw = random || Math.random;
	const ivs = rollIvs(draw);
	// Run & Bun guarantees three perfect IVs on the starter (operator ruling,
	// 2026-08-28), and /run/identity — this function's scripted-event caller —
	// serves exactly the gift class that carries the guarantee. The stats are
	// picked from the same draw stream, so the same seed still authors the
	// same identity. Wild rolls pass no opts and keep the plain die: across
	// 72 banked runs the un-guaranteed starter came out 59 times with zero
	// perfect IVs and never with three, which is how this went unnoticed.
	const perfect = (opts && opts.perfectIvs) || 0;
	if (perfect > 0) {
		const stats = Object.keys(IV_STATS);
		for (let i = 0; i < perfect && stats.length; i++) {
			const pick = Math.floor(Number(draw()) * stats.length);
			ivs[stats.splice(pick, 1)[0]] = 31;
		}
	}
...
/**
 * Where to find a currency the run just charged for.
 *
 * A refusal that names a price and not a source is only half an answer: the
 * run debits a Rare Candy per level over the cap and a Heart Scale per
 * relearned move, and neither is among the 28 items `fieldItems` models. The
 * workbook has held 30 Heart Scale locations and 14 Rare Candy the whole
 * time, read by nothing.
 *
 * Places, not dates. Inferring an unlock from a place name is what
 * `moveObtainableAt` refuses to do for TMs, and for the same reason.
 */
function whereToFind(run, item) {
	const oracle = getProfile(run.profileId).oracle;
	if (!oracle.currencySources) return '';
	const places = [];
	for (const source of oracle.currencySources(item)) {
		if (source.place && !places.includes(source.place)) places.push(source.place);
	}
	if (!places.length) return '';
	const shown = places.slice(0, 3).join(', ');
	return ` — ${places.length} known ${places.length === 1 ? 'source' : 'sources'}: ` +
		`${shown}${places.length > 3 ? ', and ' + (places.length - 3) + ' more' : ''}`;
}
...
function createRun(options) {
	...
	return {
		...
		// Which vintage of the run map this document's orders live on. The
		// 2026-08-28 insertion showed why: position and skipped are raw
		// orders, and a map edit silently rebinds them to different fights.
		// Readers of banked documents refuse a stamp they do not recognise.
		...(profile.encounters && profile.encounters.ORDER_SCALE ?
			{orderScale: profile.encounters.ORDER_SCALE.id} : {}),
...
/**
 * How a journal note names a Pokemon: the nickname or species, with the id
 * appended only when the living box makes the bare name ambiguous. The id
 * is evidence plumbing — it stays in refusals (diagnostics) and events,
 * never in the game's own voice unless two Poochyena force it.
 */
function noteName(run, mon) {
	const twins = run.box.filter(entry => entry.status !== 'dead' &&
		entry.species === mon.species).length;
	const label = mon.nickname || mon.species;
	return twins > 1 ? `${label} (${mon.id})` : label;
}
...
/**
 * Which numbered fight an order falls on, 1-based, or null past the road's end.
 *
 * Two scales run through this project and they are easy to mistake for each
 * other. `order` counts cumulative enemy POKEMON — Leader Brawly sits at 77 —
 * while a player counts trainers, and Brawly is the 26th of 362. Every dated
 * source here is in order: LEVEL_CAPS, availability's opensAt, the HM gates.
 * Only the presentation wants trainers, so the conversion lives at the edge
 * rather than in the data.
 */
function trainerIndexOf(run, order) {
	if (order === undefined || order === null) return null;
	// ABSOLUTE over the whole visible map, not the unbeaten road. The first
	// version walked upcoming(), so the number a fight wore shrank as earlier
	// fights were beaten — Brawly was the 26th trainer at the start of a run
	// and the 6th by the time you reached him — and the skip reordering would
	// have bent it further. A player counts trainers on the MAP.
	const fights = visibleFights(run);
	for (let i = 0; i < fights.length; i++) {
		if (fights[i].order >= order) return i + 1;
	}
	return null;
}

/**
 * THE ELITE FOUR IS FOUR MEMBERS, NOT EIGHT FIGHTS.
 *
 * The map carries a single AND a double variant for each of Sidney, Phoebe,
 * Glacia and Drake, and every view treated all eight as owed — so a run had
 * to beat each member twice, in both formats, four of them in doubles. The
 * game asks for four fights: the player takes TWO members as singles and TWO
 * as doubles, free choice of which (operator, 2026-09-22).
 *
 * So the road drops: the other variant of a member already fought, and the
 * format whose quota of two is already spent. The choice is the act of
 * fighting — nothing is declared in advance, which is how the game plays it.
 *
 * This lives in `upcoming` and not in `visibleFights` on purpose: the map's
 * numbering must not move under a run. `trainerIndexOf` counts a fight's
 * place over the WHOLE visible map, and a map that shrank as the Elite Four
 * were chosen would renumber every fight behind it mid-run — the two-order-
 * scales hazard this repository has been bitten by before.
 */
const E4_MEMBER = /^(Elite Four (?:Sidney|Phoebe|Glacia|Drake))(Double)?$/;
const E4_PER_FORMAT = 2;

/**
 * Which of the Four have been fought, and in which format — read from the
 * LOG, not from the position.
 *
 * `beatenOrders` is every order at or below the position, which is right for
 * a road walked in order and wrong here: beating a member's DOUBLE (the later
 * of its two entries) would mark its single beaten too, so passing two
 * members spent all four quota slots at once and retired the other two
 * members entirely. The log names the trainer actually fought.
 */
function eliteFourClosed(run) {
	const fought = new Map();
	let singles = 0;
	let doubles = 0;
	for (const entry of run.log || []) {
		const command = entry.command || {};
		if (command.kind !== 'beat') continue;
		const named = E4_MEMBER.exec(command.trainer || '');
		if (!named || fought.has(named[1])) continue;
		fought.set(named[1], true);
		if (named[2]) doubles += 1;
		else singles += 1;
	}
	return {fought, singlesSpent: singles >= E4_PER_FORMAT, doublesSpent: doubles >= E4_PER_FORMAT};
}

/**
 * The other format of the same Elite Four member — 'Elite Four Sidney' for
 * 'Elite Four SidneyDouble' and back — or null for any other fight. A run
 * walled at one format of a member may take the other (operator, 2026-09-22:
 * two singles and two doubles, free choice).
 */
function eliteFourCounterpart(trainer) {
	const named = E4_MEMBER.exec(trainer || '');
	if (!named) return null;
	return named[2] ? named[1] : named[1] + 'Double';
}

/** Whether this Elite Four variant is no longer on the road: its member is done, or its format is spent. */
function eliteFourSettled(state, trainer) {
	const named = E4_MEMBER.exec(trainer);
	if (!named) return false;
	if (state.fought.has(named[1])) return true;
	return named[2] ? state.doublesSpent : state.singlesSpent;
}

function upcoming(run, count) {
	// Unbeaten, not merely ahead. A skipped OPTIONAL fight is never owed, so
	// once declared skipped it is simply not the road. A skipped DELAYABLE
	// fight is owed — but WHERE it sorts is what makes the skip mean
	// anything. The old rule sorted it first always, which made a skip a
	// no-op: the play surface offers upcoming[0], so a skipped Gavi was
	// offered again on the very next cycle and the road never moved. Now the
	// declaration is honoured in both directions. AHEAD of the run, a
	// skipped fight sorts behind the road — that is what skipping IS, per
	// the operator's ruling on Gavi: he is meant to be passed and taken
	// after the museum grunts. Once the run has moved PAST it, the debt
	// sorts first again: the player went and did the other thing, and the
	// owed fight is now the nearest thing left standing.
	const beaten = beatenOrders(run);
	const skipped = new Set(run.skipped || []);
	const optional = getProfile(run.profileId).optionalFights || [];
	const elite = eliteFourClosed(run);
	const road = visibleFights(run)
		.filter(fight => !beaten.has(fight.order) &&
			!(skipped.has(fight.order) && optional.includes(fight.trainer)) &&
			!eliteFourSettled(elite, fight.trainer));
	// A debt must not interrupt a GAUNTLET. The museum Aqua Grunts are back
	// to back with no healing between — the game gives no way to leave and
	// fight Gavi in the middle — and consecutive fights of one gauntlet share
	// a name stem ("Team Aqua Grunt Museum #1", "#2"). While the fight just
	// beaten and the next forward fight share that stem, the road stays
	// forward and the debt keeps waiting.
	const stem = name => String(name).replace(/\s*#?\d+$/, '');
	const atPosition = visibleFights(run).find(fight => fight.order === run.position);
	const firstForward = road.find(fight =>
		!skipped.has(fight.order) && fight.order > run.position);
	const midGauntlet = !!(atPosition && firstForward &&
		stem(atPosition.trainer) === stem(firstForward.trainer));
	return road
		.sort((a, b) => {
			const defer = fight => (skipped.has(fight.order) &&
				(fight.order > run.position || midGauntlet)) ? 1 : 0;
			return defer(a) - defer(b) || a.order - b.order;
		})
		.slice(0, count || 5);
}
 ...
 *
 * `beaten` is derived from position, not stored — beating order #337 means the
...
function encountersOn(run, map) {
	...
	// A method whose HM is not in hand yet is still this route's future, but
	// it must be MARKED: offering surf before Surf exists sends the player
	// into a refusal they cannot act on.
	const methodGate = method => {
		if (!profile.oracle.methodOpensAt) return null;
		const gate = profile.oracle.methodOpensAt(method, found.map);
		return gate !== null && run.position + 1 < gate ? gate : null;
	};
	const mons = found.mons.map(mon => Object.assign(
		{owned: owned.has(mon.species)},
		mon,
		methodGate(mon.method) !== null ? {methodGated: methodGate(mon.method)} : {},
	));
...
function unusedRoutes(run, options) {
	...
	for (const members of groups.values()) {
		...
		if (opensAt !== undefined) {
			...
			// No date. availability.json anchors a location to its FIRST
			// TRAINER, so anywhere without one — Oldale Town, the whole Safari
			// Zone, Fiery Path, Sky Pillar, Altering Cave — never gets a date
			// at all. Leaving `open` unset made that mean "closed forever" to
			// every consumer below, because they all filter on `route.open`
			// and undefined is falsy. 18 of 69 locations vanished from every
			// answer, 12 of them with walk encounters, including one of the
			// first places a run can catch anything.
			//
			// Undated is not closed and it is not open — it is UNKNOWN, and it
			// says so, exactly as an undated TM does rather than being sold as
			// current prep.
			entry.undated = true;
		}
			...
			if (held.for && profile.oracle.methodOpensAt) {
				const slots = members.flatMap(map =>
					profile.oracle.encountersOn(map.name).mons.filter(mon => mon.species === held.for)
						.map(mon => Object.assign({map: map.map}, mon)));
				entry.held.ready = slots.some(slot => {
					const gate = profile.oracle.methodOpensAt(slot.method, slot.map);
		...
		entry.best = pool
			...
			.map(mon => {
				...
				// encountersOn marked the slot with its own map's gate.
				const gate = mon.methodGated !== undefined ? mon.methodGated : profile.oracle.methodOpensAt ?
...
function learnable(run, id, options) {
	...
	for (const move of Object.keys(all)) {
		...
		// The nurse's "Remember a move" covers an egg move AND one from a
		// previous learnset, and charges a Heart Scale for either; only the
		// level-up's own standing prompt is free (mon.prompted).
		const prompted = (mon && mon.prompted ? mon.prompted : []).includes(move);
		if (!prompted && sources.some(s =>
			isEggSource(s) || (s.level !== undefined && s.level <= atLevel))) {
			entry.scale = true;
		}
		if (prompted) entry.prompted = true;
		// Nor is it free when it comes from a TM: a TM is a one-time item in
		// this fork, so the row names the TM it spends and whether the bag
		// holds one. A surface that offers what the bag cannot pay for sends
		// the player into a refusal (advisors-recommend-only-the-possible).
		if (!entry.scale && profile.oracle.tmFor && sources.every(s =>
			s.source === 'teachable' || (s.level !== undefined && s.level > atLevel))) {
			const tutored = profile.oracle.tutorTeaches && profile.oracle.tutorTeaches(move);
			const tm = tutored ? null : profile.oracle.tmFor(move);
			if (tm) {
				entry.tm = tm.name;
				entry.owned = (run.bag[tm.name] || 0) > 0;
			}
			// A tutor the road has not reached is not "now" either (the teach
			// rule refuses it): the row says when it opens.
			const opens = tutored && profile.oracle.tutorOpensAt ? profile.oracle.tutorOpensAt(move) : undefined;
			if (typeof opens === 'number' && run.position + 1 < opens) entry.tutorOpensAt = opens;
		}
		if ((gated && soonest > atLevel) || entry.tutorOpensAt !== undefined) later.push(entry);
		else now.push(entry);
	}
	const byName = (a, b) => a.move.localeCompare(b.move);
	const when = entry => (entry.level === undefined ? Number.MAX_SAFE_INTEGER : entry.level);
	return {now: now.sort(byName), later: later.sort((a, b) => when(a) - when(b) || byName(a, b))};
}

/**
 * The Mega form a body takes holding this item, or null.
 *
 * Neither engine performs a Mega Evolution, so the ENEMY's Megas are stored
 * already evolved, holding their stone (tests/mega_forms.test.js) — the
 * evolution resolves before any move, so the evolved form's Speed is what
 * orders turn one. The player's are modelled the same way, which the run had
 * never done at all: a run reached the Elite Four with about twenty Mega
 * Stones loose in the bag and a six holding Pixie Plates, against bosses that
 * have fielded a Mega since the third gym.
 */
function megaFormOf(species, item) {
	const calc = require('../calc');
	if (!item || !calc.MEGA_STONES || calc.MEGA_STONES[item] !== species) return null;
	const suffix = / X$/.test(item) ? '-Mega-X' : / Y$/.test(item) ? '-Mega-Y' : '-Mega';
	const found = calc.Generations.get(8).species.get(calc.toID(species + suffix));
	return found ? {species: found.name, ability: Object.values(found.abilities || {})[0] || null} : null;
}

/** Whether the run holds the Mega Ring for a fight at `atOrder` (or right now). */
function megaRingHeld(run, atOrder) {
	const profile = getProfile(run.profileId);
	const ring = profile.oracle && profile.oracle.megaRingOpensAt ? profile.oracle.megaRingOpensAt() : null;
	if (ring === null || ring === undefined) return false;
	// The ring is handed over for BEATING that fight, so it is held for any
	// fight after it, and now if the road is already past it.
	return run.position >= ring || (atOrder !== undefined && atOrder !== null && atOrder > ring);
}

/**
 * Specs with the run's Mega applied. ONE a fight (ruling the-player-megas):
 * `megas: 'first'` evolves the first member holding its own stone, in party
 * order, and leaves any other stone-holder in its base form. The box matrix
 * asks for `'each'`, because a grid row is that body considered alone.
 */
function withMegas(run, specs, atOrder, mode) {
	if (mode === 'none' || !megaRingHeld(run, atOrder)) return specs;
	let spent = false;
	return specs.map(spec => {
		if (spent && mode !== 'each') return spec;
		// In the box matrix a row is a body considered ALONE, and the question
		// it answers is "who should be in the six" — so a body whose stone is
		// in the bag is rated as the Mega it would be handed. Rated only as it
		// stands, Mega-capable bodies never made the six and their stones never
		// left the bag.
		const stone = mode === 'each' && !megaFormOf(spec.species, spec.item) ? stoneInBag(run, spec.species) : null;
		const form = megaFormOf(spec.species, stone || spec.item);
		if (!form) return spec;
		spent = true;
		return Object.assign({}, spec, {species: form.species, ability: form.ability || spec.ability,
			megaOf: spec.species}, stone ? {item: stone} : {});
	});
}

/** The Mega Stone for this species that the bag holds, or null. */
function stoneInBag(run, species) {
	const stones = require('../calc').MEGA_STONES || {};
	return Object.keys(run.bag || {}).find(item => stones[item] === species && run.bag[item] > 0) || null;
}
...
function partySpecs(run, options) {
	...
	return withMegas(run, run.party.map(id => {
		const mon = requireMon(run, id);
		...
		const level = projected === null ? mon.level : Math.max(mon.level, projected);
		const learned = projectMoves(getProfile(run.profileId), mon, level);
		return {
			species: mon.species,
			level,
			nature: mon.nature,
			...
			item: mon.item,
			moves: learned.moves,
			...(learned.pending.length ? {pendingMoves: learned.pending} : {}),
			ivs: mon.ivs,
		};
	}), opts.atOrder, opts.megas || 'first');
}

/**
 * The moveset a Pokemon would actually HAVE at a projected level.
 *
 * Projection raised the level and left `moves` exactly as the box recorded
 * them, so a Chimchar caught at L5 and planned at L12 fought with its L5
 * moveset — Scratch, Leer, Ember — while the real thing learns Mach Punch at
 * L10. Every matchup, plan and safety answer was computed against a moveset
 * the player would not have.
 *
 * Free slots are filled with what the level-ups give, newest first. A FULL
 * moveset is left alone: the game asks which move to forget and only the
 * player knows what they chose, so guessing a replacement would trade one
 * wrong answer for another. Those are returned as `pending` instead, so the
 * caller can say "you would also have learned this" rather than silently
 * claiming it either way.
 */
function projectMoves(profile, mon, level) {
	const known = (mon.moves || []).slice();
	if (level <= mon.level) return {moves: known, pending: [], taught: []};
	const byLevel = profile.oracle.levelUpMoves(mon.species) || [];
	// Dedupe. A learnset can list the same move at more than one level, and
	// the first version of this checked only the ORIGINAL moveset, so a move
	// learned twice in the range was added twice and the engine rejected the
	// whole state: "moves repeats move Bite".
	const newlyLearned = [];
	for (const pair of byLevel) {
		if (pair[0] <= mon.level || pair[0] > level) continue;
		if (known.includes(pair[1]) || newlyLearned.includes(pair[1])) continue;
		newlyLearned.push(pair[1]);
	}
	if (!newlyLearned.length) return {moves: known, pending: [], taught: []};
	const moves = known.slice();
	const pending = [];
	const taught = [];
	for (const move of newlyLearned) {
		if (moves.length < 4) { moves.push(move); taught.push(move); } else pending.push(move);
	}
	return {moves, pending, taught};
}
 ...
 */
/**
 * Enemy sets whose danger scales with their own MISSING health.
 *
 * Reversal and Flail hit hardest at 1 HP and Endeavor equalises to it, so an
 * item or ability that guarantees surviving there — Focus Sash, a pinch
 * berry, Sturdy, Endure — turns "nearly dead" into the most dangerous state
 * the fight has. The forecast's fair-dice sampler is KNOWN blind to that
 * scaling: the pinned provider's bundle carries the move name and no
 * HP-threshold power formula, while the engine that plays the fight
 * implements it (calc gen789, case 'Reversal'). Battle Girl Lilith's sash
 * Mankey was forecast "worst sampled branch loses 1" fourteen times and swept
 * six Pokemon in twelve of them; over one measured batch, 23 of 30 such
 * fights lost two or more MORE than the samples predicted, and excluding them
 * repaired the calibration curve entirely (81.5% -> 100% in its worst
 * bucket).
 *
 * So a plan against one of these names it. The verdict may still read clean —
 * the samples are still evidence — but a clean sample against a set the
 * sampler cannot price is not the same claim as a clean sample elsewhere, and
 * the difference belongs in front of the player, not in a ledger.
 */
const THRESHOLD_MOVES = ['Reversal', 'Flail', 'Endeavor'];
const THRESHOLD_ANCHORS = ['Focus Sash', 'Custap Berry', 'Salac Berry'];
function thresholdThreatsIn(fight) {
	const threats = [];
	for (const mon of fight.party || []) {
		const moves = mon.moves || [];
		const held = THRESHOLD_MOVES.filter(move => moves.includes(move));
		if (!held.length) continue;
		const anchor = THRESHOLD_ANCHORS.includes(mon.item) ? mon.item :
			mon.ability === 'Sturdy' ? 'Sturdy' :
				moves.includes('Endure') ? 'Endure' : null;
		if (anchor) threats.push({species: mon.species, move: held.join('/'), holds: anchor});
	}
	return threats;
}
...
function planNext(run, options) {
	...
	plan.thresholdThreats = thresholdThreatsIn(fight);
	plan.fightNumber = trainerIndexOf(run, fight.order);
	// The dossier's tech flags: the punishes and tools this kit carries that
	// a plan must respect BEFORE any move choice matters — Sturdy+Custap,
	// Weakness Policy, absorbers. Precomputed offline; a lookup here.
	const profileForDossier = getProfile(run.profileId);
	const dossierRow = profileForDossier.oracle && profileForDossier.oracle.fightDossierOf ?
		profileForDossier.oracle.fightDossierOf(fight.trainer) : null;
	plan.dossierTech = dossierRow ? dossierRow.tech : [];
	return plan;
}
 ...
 */
function boxMatrix(run, trainer, options) {
	const planner = require('./planner');
	const ahead = upcoming(run, 1);
	...
	const specs = partySpecs(
		...
		{atOrder: fight.order, megas: (options && options.megas) || 'each'});
	...
	matrix.box = alive.map((mon, i) => ({
		...
		level: specs[i].level,
		from: mon.level,
		// The Mega this row was rated as, when it was rated as one. A six may
		// hold several such bodies and field only ONE, so a set score has to
		// know which rows are a Mega's before it can choose between them.
		mega: specs[i].megaOf ? specs[i].species : null,
	}));
	matrix.projection = {
		applied: cap !== null,
		cap,
	...
	matrix.fightNumber = trainerIndexOf(run, fight.order);
	return matrix;
}
 ...
 */
/**
 * A spec with its held item changed — and its FORM with it, when the item is
 * this body's Mega Stone or replaces it. A give row priced without this would
 * value Gyaradosite as an item that does nothing, which is how a box came to
 * the Elite Four with its stones in the bag. One Mega a fight: if another
 * member of the party already holds its own stone, this one stays as it is.
 */
function itemSwapped(run, mon, spec, item, order) {
	const base = Object.assign({}, spec, {species: mon.species, ability: mon.ability, item});
	delete base.megaOf;
	const taken = run.party.some(id => {
		if (id === mon.id) return false;
		const other = findMon(run, id);
		return other && megaFormOf(other.species, other.item) !== null;
	});
	return taken ? base : withMegas(run, [base], order, 'first')[0];
}
...
function upgradeCandidates(run, mon, spec, base, order) {
	...
	for (const entry of learnable(run, mon.id, {atLevel: spec.level}).now) {
		...
		// anchorOpen, not `gate <= order`. The numeric test admits a move whose
		// guard is the very fight being prepared for: TM16 Seismic Toss is
		// Brawly's own gym reward and carries his order, 77, so advising it for
		// Brawly told a player to use a prize they can only win by first
		// winning without it. preFightOpportunities already asks the run-aware
		// question — has the guard actually been beaten — and these two views
		// of the same fact disagreed.
		const datedTeachRoute = teachableRoute && gate !== null && anchorOpen(run, gate);
		...
		// Already knows it. Never offered before because projection copied the
		// box moveset verbatim, so a level-up move could not already be there;
		// now that levelling teaches, "teach Bite" to a Pokemon that learned
		// Bite at L13 is both useless advice and an invalid build — the engine
		// refuses a moveset that repeats a move.
		if ((spec.moves || []).indexOf(entry.move) !== -1) continue;
	...
	for (const item of Object.keys(run.bag).sort()) {
		...
		list.push({
			...
			spec: itemSwapped(run, mon, spec, item, order),
			item,
		});
	}
	...
	if (order !== undefined && profile.oracle.itemsObtainableBy) {
		...
		for (const pickup of profile.oracle.itemsObtainableBy(order)) {
			...
			// The advisor plans AHEAD (opensAt <= the fight's order); the
			// pickup button gates on PROGRESS (anchorOpen: the guard fight
			// beaten). Both are right, and the row used to hide which one it
			// was, so a player went looking for a button that was not there.
			// A pickup not yet reachable says when, in the split sheet's words.
			const reachable = anchorOpen(run, pickup.opensAt);
			list.push({
				kind: 'pickup',
				detail: `${pickup.name} (pickup @ ${pickup.location}` +
					(reachable ? '' : ` · from #${pickup.opensAt}`) + ')' +
					(spec.item ? ` over ${spec.item}` : ''),
				spec: itemSwapped(run, mon, spec, pickup.name, order),
				item: pickup.name,
				opensAt: pickup.opensAt,
				reachable,
			});
		}
	}
	...
	for (const step of profile.oracle.evolutionsOf(mon.species)) {
		if (!require('./dossier').meetsRequirement(step, mon.species, spec.level, mon.ivs, mon.nature)) continue;
		const eligible =
 ...
 */
/**
 * Which party members a crit can kill in a fight, from the same cells the
 * board colours. One shared definition of "exposed" so the readiness strip,
 * the advisor and the safety path can never disagree about the same risk.
 */
/**
 * Is this pairing lethal in the worst case? Their crit killing us is only
 * half the question: a Pokemon that outspeeds and KOs on its WORST roll is
 * never hit at all, so the crit it would have taken never happens. That is
 * why teaching a move can buy survival even though it changes nothing about
 * our defenses — and it is the honest reading of "worst case", because both
 * halves assume the bad branch (our minimum roll, their crit).
 *
 * Deliberately conservative: a speed TIE is not outspeeding, multi-hit
 * survivals (Sturdy, Sash) and switch-ins are not credited, and — the one
 * that makes this sound — a killer whose crit comes from a PRIORITY move
 * cancels the credit entirely, because priority ignores Speed. Mach Punch
 * off a Hitmontop is exactly this case and it appears in this game's real
 * exposure lists.
 *
 * HONESTY NOTE: a scan of sampled fights across the whole run map found ZERO
 * pairings where this credit currently fires — the condition needs a Pokemon
 * that both floor-KOs its opponent and would die to that opponent's crit,
 * which the early game's level caps do not produce. The rule is kept because
 * it is the correct reading of the exchange (a Pokemon that never gets hit
 * cannot be crit) and it will matter once a run fields real attackers, but it
 * is exercised only by the direct unit test below, never yet by live data.
 * If it ever changes a verdict in play, that is new behaviour worth checking.
 */
function pairingLethal(cell) {
	if (!cell || !cell.them || !cell.them.critKO) return false;
	// Their priority move swings first no matter how fast we are.
	if (cell.them.critPriority) return true;
	const outspeedKO = cell.speed === 'faster' && cell.us && cell.us.min >= 1;
	return !outspeedKO;
}

function critExposure(grid, slotCount) {
	const exposed = [];
	for (let slot = 0; slot < slotCount; slot += 1) {
		const killers = grid
			.filter(cell => pairingLethal(cell.versus[slot]))
			.map(cell => ({
				enemy: cell.enemy.species,
				move: cell.versus[slot].them.critMove || cell.versus[slot].them.move,
			}));
		if (killers.length) exposed.push({slot, killers});
	}
	return exposed;
}

/**
 * The preparation graph's first honest query: of everything this run can do
 * BEFORE the next fight, what changes who a crit can kill — and what does it
 * cost?
 *
 * The upgrade advisor answers a different question ("what raises damage"), and
 * a party can gain damage while staying just as dead. This walks the same
 * candidate edges — teach a move, hold a bag item, level to the cap — and
 * scores each one solely by whether it removes a lethal branch, then reports
 * the price in the resources the run cannot get back.
 *
 * It deliberately does NOT invent catches: a route's encounter is random, so
 * "catch something on Route 104" is a lottery ticket, not a plan. Open routes
 * are reported as unpriced leads instead.
 */
/**
 * What a candidate actually CHANGES, as a diff against the base spec rather
 * than a whole replacement spec. Two candidates for the same Pokemon each
 * carry a full spec built from the same base, so merging the specs would
 * silently drop one edit; merging the diffs composes them.
 */
function candidateChange(base, candidate) {
	if (candidate.kind === 'give' || candidate.kind === 'pickup') {
		return {kind: candidate.kind, item: candidate.spec.item};
	}
	if (candidate.kind === 'teach') {
		const before = base.moves || [];
		const after = candidate.spec.moves || [];
		return {
			kind: 'teach',
			add: after.filter(move => before.indexOf(move) === -1)[0],
			drop: before.filter(move => after.indexOf(move) === -1)[0],
		};
	}
	return {kind: candidate.kind, spec: candidate.spec};
}

/** Apply composed changes to a spec, in order, so later edits see earlier ones. */
function applyChanges(base, changes) {
	let spec = Object.assign({}, base, {moves: (base.moves || []).slice()});
	for (const change of changes) {
		if (change.kind === 'give' || change.kind === 'pickup') {
			spec = Object.assign({}, spec, {item: change.item});
		} else if (change.kind === 'teach' && change.add) {
			const moves = spec.moves.slice();
			// Already knows it: nothing to do. The push path always checked
			// this; the REPLACE path did not, so once projection started
			// teaching level-up moves the advisor would propose "Bite over
			// Tackle" to a Pokemon that had just learned Bite, and hand the
			// engine [Bite, Sand Attack, Odor Sleuth, Bite] — which it
			// rightly refused: "moves repeats move Bite".
			if (moves.indexOf(change.add) !== -1) continue;
			const at = change.drop ? moves.indexOf(change.drop) : -1;
			if (at !== -1) moves[at] = change.add;
			else moves.push(change.add);
			spec = Object.assign({}, spec, {moves});
		} else if (change.spec) {
			spec = Object.assign({}, spec, change.spec);
		}
	}
	return spec;
}

/** Two changes conflict when they claim the same single slot. */
function changesConflict(a, b) {
	const held = change => change.kind === 'give' || change.kind === 'pickup';
	// One Pokemon holds one item.
	if (held(a) && held(b)) return true;
	// Two teaches that evict the same move are the same slot twice.
	if (a.kind === 'teach' && b.kind === 'teach') {
		if (a.add === b.add) return true;
		if (a.drop && b.drop && a.drop === b.drop) return true;
	}
	return false;
}

/** The held items an option's steps would claim from the one shared bag. */
function optionItems(option) {
	return (option.steps || [])
		.filter(step => step.kind === 'give' || step.kind === 'pickup')
		.map(step => step.spec.item)
		.filter(Boolean);
}

/**
 * The run owns ONE of each item: two Pokemon cannot both hold the same Oran
 * Berry, and an option that claims two of something the bag holds once is not
 * a plan. Counted per item so a duplicate claim inside one option is caught
 * as well as a claim across Pokemon.
 */
function affordableFromBag(items, ledger) {
	const claimed = {};
	for (const item of items) {
		claimed[item] = (claimed[item] || 0) + 1;
		if ((ledger[item] || 0) < claimed[item]) return false;
	}
	return true;
}

function spendFromBag(items, ledger) {
	for (const item of items) {
		if (ledger[item]) ledger[item] -= 1;
	}
	return ledger;
}

/**
 * The options this run can actually fund, and NOTHING else.
 *
 * There is deliberately no fallback to the unfunded list: an earlier version
 * fell back when nothing affordable helped, which handed one Focus Sash to
 * three different Pokemon and told the player each of them was now safe. If
 * the bag cannot pay for it, it is not a plan — the free answers (who leads
 * safely, who can face what) are the honest reply instead.
 */
function affordableOptions(improving, ledger) {
	return improving.filter(option => affordableFromBag(optionItems(option), ledger));
}

function safetyPath(run, trainer) {
	const planner = require('./planner');
	if (!run.party.length) {
		throw new Error('the party is empty: add Pokemon before asking what makes a fight survivable');
	}
	const ahead = upcoming(run, 1);
	const named = trainer || (ahead.length ? ahead[0].trainer : null);
	if (!named) throw new Error('nothing ahead in the run map to make safe');
	const fight = planner.getFight(named, run.profileId);
	const specs = partySpecs(run, {atOrder: fight.order});
	const baseline = planner.matchup({trainer: named, playerParty: specs, profileId: run.profileId});
	const before = critExposure(baseline.grid, specs.length);

	const answer = {
		trainer: named,
		order: fight.order,
		exposed: before.map(entry => ({
			id: run.party[entry.slot],
			species: requireMon(run, run.party[entry.slot]).species,
			killers: entry.killers,
		})),
		steps: [],
		coverage: [],
		saferLeads: [],
	...
	if (!before.length) return answer;

	// One candidate at a time, each re-scored against the same board. A step
	// only earns its place by removing at least one lethal branch.
	// A candidate only ever touches ONE Pokemon's row, so combinations across
	// different Pokemon are independent and never need a joint re-score. Only
	// combinations on the SAME Pokemon do — which is what keeps this search
	// tractable: per-mon depth-2 beam, not a subset lattice over the party.
	const PAIR_BEAM = 4;
	const exposedSlots = new Set(before.map(entry => entry.slot));
	const perMon = [];
	run.party.forEach((id, slot) => {
		if (!exposedSlots.has(slot)) return;
		const mon = requireMon(run, id);
		const base = baseline.grid.map(cell => cell.versus[slot]);
		const wasLethal = base.filter(pairingLethal).length;
		const candidates = upgradeCandidates(run, mon, specs[slot], base, fight.order);
		const scored = [];
		const score = changes => {
			const row = planner.matchup({
				trainer: named,
				playerParty: [applyChanges(specs[slot], changes)],
				profileId: run.profileId,
			}).grid.map(cell => cell.versus[0]);
			return row.filter(pairingLethal).length;
		};
		candidates.list.forEach(candidate => {
			const change = candidateChange(specs[slot], candidate);
			const stillLethal = score([change]);
			scored.push({candidate, change, stillLethal, steps: [candidate]});
		});
		// Pair only the most promising singles: a pair whose members both fail
		// alone can still succeed together (coverage of different enemies), so
		// the beam is ranked by remaining lethality, not by "helped alone".
		const beam = scored.slice()
			.sort((a, b) => a.stillLethal - b.stillLethal ||
				a.candidate.detail.localeCompare(b.candidate.detail))
			.slice(0, PAIR_BEAM);
		for (let i = 0; i < beam.length; i += 1) {
			for (let j = i + 1; j < beam.length; j += 1) {
				if (changesConflict(beam[i].change, beam[j].change)) continue;
				const changes = [beam[i].change, beam[j].change];
				const stillLethal = score(changes);
				// A pair earns its extra cost only by beating BOTH halves.
				if (stillLethal >= Math.min(beam[i].stillLethal, beam[j].stillLethal)) continue;
				scored.push({stillLethal, changes,
					steps: [beam[i].candidate, beam[j].candidate]});
			}
		}
		const improving = scored.filter(entry => entry.stillLethal < wasLethal);
		if (improving.length) perMon.push({id, slot, mon, wasLethal, improving});
	});

	// Compose across Pokemon against one shared bag: the run owns a single
	// Oran Berry, so two Pokemon cannot both hold it. Cheapest-first, and the
	// mon that gains more from an item claims it.
	const bagLedger = Object.assign({}, run.bag);
	perMon.sort((a, b) => (b.wasLethal - a.wasLethal) || a.slot - b.slot);
	perMon.forEach(entry => {
		const pool = affordableOptions(entry.improving, bagLedger);
		if (!pool.length) return;
		pool.sort((a, b) => a.stillLethal - b.stillLethal ||
			a.steps.length - b.steps.length ||
			a.steps[0].detail.localeCompare(b.steps[0].detail));
		const best = pool[0];
		spendFromBag(optionItems(best), bagLedger);
		answer.steps.push({
			id: entry.id,
			species: entry.mon.species,
			// A path, not a single edit: each element is one thing to do.
			path: best.steps.map(step => ({kind: step.kind, detail: step.detail})),
			detail: best.steps.map(step => step.detail).join(' + '),
			kind: best.steps.length === 1 ? best.steps[0].kind : 'path',
			removes: entry.wasLethal - best.stillLethal,
			remaining: best.stillLethal,
			cost: best.steps.map(step =>
				step.kind === 'give' || step.kind === 'pickup' ? 'the item leaves the bag' :
					step.kind === 'teach' ? 'a move slot' : 'unpriced')
				.filter((cost, index, all) => all.indexOf(cost) === index)
				.join(' · '),
		});
	});
	answer.steps.sort((a, b) => b.removes - a.removes ||
		a.species.localeCompare(b.species) || a.detail.localeCompare(b.detail));

	// The assignment answer — usually the only real one. When their crit
	// simply outdamages a Pokemon's whole HP bar, no move and no item fixes
	// it; what fixes it is not sending that Pokemon in. For each of their
	// Pokemon that can kill someone, name who can face it and live.
	answer.coverage = baseline.grid
		.map((cell, enemyIndex) => {
			const killed = [];
			const safe = [];
			run.party.forEach((id, slot) => {
				const pairing = cell.versus[slot];
				if (!pairing) return;
				const mon = requireMon(run, id);
				const name = mon.nickname || mon.species;
				if (pairingLethal(pairing)) killed.push(name);
				else safe.push({name, crit: Math.round((pairing.them.critMax || 0) * 100)});
			});
			if (!killed.length) return null;
			safe.sort((a, b) => a.crit - b.crit);
			return {
				enemy: cell.enemy.species,
				enemyIndex,
				kills: killed,
				answers: safe.map(entry => entry.name),
				bestAnswer: safe.length ? safe[0].name : null,
				bestAnswerCrit: safe.length ? safe[0].crit : null,
			};
		})
		.filter(Boolean);

	// The cheapest edge in the graph costs nothing at all: send someone else
	// out first. Read straight off the same board — a party member none of
	// their Pokemon can crit-kill is a safe lead, and the run already owns it.
	const exposedIds = new Set(answer.exposed.map(entry => entry.id));
	if (exposedIds.has(run.party[0])) {
		answer.saferLeads = run.party
			.map((id, slot) => ({id, slot}))
			.filter(entry => entry.slot !== 0 && !exposedIds.has(entry.id))
			.map(entry => {
				const mon = requireMon(run, entry.id);
				return {id: entry.id, species: mon.species, nickname: mon.nickname || null};
			});
	} else {
		answer.saferLeads = [];
	}

	// Routes still unspent are the graph's unpriced edges: a real option, but
	// a random one. Named, never scored.
	const rules = encounterRules(run);
	answer.openRoutes = unusedRoutes(run).routes
...
/**
 * What each cure item answers. A held item is not only a bar of HP: against a
 * team built on Thunder Wave and Attract the item that matters is the one that
 * gives a turn back, and the upgrade list could not see those at all because
 * its only currency was damage.
 */
const CURES = {
	'cheri berry': ['par'],
	'chesto berry': ['slp'],
	...
	'lum berry': ['par', 'slp', 'psn', 'tox', 'brn', 'frz', 'confusion'],
	'mental herb': ['infatuation'],
};

/** Abilities that inflict on contact, so a team's threat is not only its moves. */
const ABILITY_THREATS = {
	static: ['par'], flamebody: ['brn'], poisonpoint: ['psn'], poisontouch: ['psn'],
	cutecharm: ['infatuation'], effectspore: ['psn', 'par', 'slp'],
};
...
function abilityId(name) {
	return String(name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
}

/**
 * Every status this fight can inflict, and how many of its Pokemon can inflict
 * it. Counted per POKEMON rather than per move, because two Thunder Waves on
 * one Pokemon is still one thing to play around.
 */
function fightStatusThreats(state) {
	const ai = require('../ai');
	const counts = {};
	for (const mon of state.sides.ai.party) {
		const seen = new Set();
		for (const entry of mon.moves || []) {
			const id = abilityId(entry.name || entry);
			if (id === 'attract') seen.add('infatuation');
			else if (ai.STATUS_BY_MOVE[id]) seen.add(ai.STATUS_BY_MOVE[id]);
			else if (ai.PURE_CONFUSION_MOVE_IDS.has(id)) seen.add('confusion');
		}
		for (const status of ABILITY_THREATS[abilityId(mon.ability)] || []) seen.add(status);
		for (const status of seen) counts[status] = (counts[status] || 0) + 1;
	}
	return counts;
}

/** How many of this fight's Pokemon an item takes an answer to. */
function statusAnswered(item, threats) {
	const cures = CURES[itemId(item)];
	if (!cures) return 0;
	return cures.reduce((total, status) => total + (threats[status] || 0), 0);
}
...
function adviseUpgrades(run, trainer) {
	...
	// What this fight can do to a turn, as opposed to a health bar.
	const threats = fightStatusThreats(planner.buildFightState({
		trainer: named,
		playerParty: specs,
		profileId: run.profileId,
	}).state);

	// The dossier's named answers for THIS fight: an upgrade spent on one of
	// them is the plan's own recommendation (stats stacked on the right
	// names), so the row is marked and near-ties resolve toward it.
	const upgradeProfile = getProfile(run.profileId);
	const upgradeDossier = upgradeProfile.oracle && upgradeProfile.oracle.fightDossierOf ?
		upgradeProfile.oracle.fightDossierOf(fight.trainer) : null;
	const upgradeAnswers = new Set(upgradeDossier ? [
		...(upgradeDossier.keyBox || []),
		...(upgradeDossier.mons || []).flatMap(mon => mon.topAnswers || []),
	] : []);
	const evolveForUpgrades = require('./dossier').evolveTo;
	...
	run.party.forEach((id, slot) => {
		...
		for (const candidate of candidates.list) {
			considered += 1;
			// Teaching a move changes which moves we know and nothing else —
			// not a stat, not a type, not the item — so the enemy's damage
			// INTO us is the same number the baseline already computed. It is
			// also subtracted from itself inside upgradeDelta, so it could
			// never have moved the answer. Computing it anyway was 43.6% of
			// every cell and about 16% of a whole advise.
			//
			// Only teach. A held item can change what comes back the other way
			// — Eviolite, a resist berry, Focus Sash are all in the dex — and
			// a resist berry answers ONE type, so probing a single cell and
			// reusing the verdict would read "unchanged" against an enemy that
			// does not carry that type and be wrong about the next one. An
			// evolution changes everything. Both keep paying in full.
			const spliceThem = candidate.kind === 'teach';
			const row = planner.matchup({
				trainer: named,
			...
			if (spliceThem) {
				row.forEach((cell, index) => {
					cell.them = base[index].them;
					cell.speed = base[index].speed;
				});
			}
			const delta = upgradeDelta(base, row);
			delta.statusAnswered = statusAnswered(candidate.spec.item, threats);
			// An upgrade used to have to gain a KO or add damage to be shown at
			// all, which silently deleted every defensive option: a Cheri Berry
			// moves neither number and was dropped before the player could see
			// it. Answering a status this fight actually carries is now its own
			// reason to appear. It does not outrank damage — pricing a turn
			// against a bar of HP needs status in the matchup model, which this
			// is not — but it is no longer invisible.
			if (delta.koGained - delta.koConceded <= 0 && delta.damage <= 0 &&
				!delta.statusAnswered) continue;
			// A one-shot item is not the same offer as a permanent one, and the
			// score cannot tell them apart: the grid credits a held item in every
			// enemy column, so a Chople Berry that fires once is priced as though
			// it fires six times. Fixing the score needs the run to know when an
			// item was eaten, which it does not. Naming the item's kind is what
			// can be told truthfully today, and it is the half a player acts on —
			...
			const consumable = candidate.item ? isSingleUse(candidate.item) : null;
			upgrades.push(Object.assign({kind: candidate.kind, id, detail: candidate.detail,
				keyAnswer: upgradeAnswers.has(evolveForUpgrades(mon.species,
					cap !== null ? cap : mon.level)), delta},
			consumable === null ? {} : {singleUse: consumable},
			candidate.kind === 'pickup' ? {opensAt: candidate.opensAt, reachable: candidate.reachable} : {}));
		}
	});

	upgrades.sort((a, b) =>
		(b.delta.koGained - b.delta.koConceded) - (a.delta.koGained - a.delta.koConceded) ||
		(b.keyAnswer ? 1 : 0) - (a.keyAnswer ? 1 : 0) ||
		b.delta.damage - a.delta.damage ||
		b.delta.statusAnswered - a.delta.statusAnswered ||
	...
	return {
		trainer: fight.trainer,
		order: fight.order,
		fightNumber: trainerIndexOf(run, fight.order),
		considered,
...
function adviseCatches(run, trainer) {
	...
	const scanned = unusedRoutes(run, {allProspects: true}).routes;
	// `used` only closes a route when the run's rules SAY one encounter per
	// route. This filtered on it unconditionally, so a run with the nuzlocke
	// rule off lost advice for every route it had ever caught on — the route
	// stayed legal to catch from, and the scout quietly stopped mentioning
	// it. preFightOpportunities has always asked the question correctly
	// (`!rules.onePerRoute || !route.used`); this is the same question.
	const spendsRoutes = encounterRules(run).onePerRoute;
	const all = scanned.filter(route => (!spendsRoutes || !route.used) && route.open);
	const routes = all.filter(route => !route.held);
	const held = all.length - routes.length;
	// Locations availability.json cannot date. They are not closed — nobody
	// knows — and staying silent about them made the scout claim to have
	// scanned the map when it had skipped a fifth of it, Oldale Town
	// included. Reported for the same reason `held` is: the answer does not
	// spend them and does not forget they exist.
	const undated = scanned.filter(route => route.undated && !route.used);
	// The precomputed dossier names this fight's ANSWERS — bodies that 2HKO
	// while surviving the reply, a criterion the KO-counting sort below
	// cannot see. A prospect whose evolved-at-cap form is on the list is
	// flagged and outranks everything (measured at Brawly: the named box at
	// 31 IVs took the first counterfactual wins ever; names are the half of
	// the key a catch can buy).
	const dossierRow = profile.oracle.fightDossierOf ?
		profile.oracle.fightDossierOf(fight.trainer) : null;
	const keyAnswers = dossierRow ? [...new Set(
		(dossierRow.keyBox || []).concat(
			(dossierRow.mons || []).flatMap(mon => mon.topAnswers || [])))] : [];
	const answerSet = new Set(keyAnswers);
	const evolveTo = require('./dossier').evolveTo;
	const branchOdds = require('./dossier').branchOdds;
	const branching = (species, level) => Object.keys(branchOdds(species, level)).length > 1;
	...
	for (const route of routes) {
		for (const prospect of route.best) {
			...
			catches.push({
				...
				level,
				keyAnswer: answerSet.has(evolveTo(prospect.species, cap !== null ? cap : level)) &&
					!branching(prospect.species, cap !== null ? cap : level),
				// A line that branches by the catch's own stats (Tyrogue) is a
				// roll, not a promise: each form with its chance, and the chance
				// that the roll lands on a named answer.
				...(branching(prospect.species, cap !== null ? cap : level) ? (() => {
					const odds = require('./dossier').branchOdds(prospect.species, cap !== null ? cap : level);
					return {branches: odds, keyAnswerChance: Object.keys(odds)
						.filter(form => answerSet.has(form)).reduce((sum, form) => sum + odds[form], 0)};
				})() : {}),
				kos: countKO(row, 'us'),
	...
	catches.sort((a, b) => (b.keyAnswer ? 1 : 0) - (a.keyAnswer ? 1 : 0) ||
		b.newAnswers - a.newAnswers || b.kos - a.kos ||
	...
	return {
		...
		undated: {
			...
			routes: undated.map(route => route.name),
			why: 'no first trainer stands here, so availability.json cannot date it',
		},
...
function rollEncounter(run, options) {
	...
	// its table weight.
	// A species the hack removed never turns up, whatever a table says. The
	// author's workbook lists Sunkern, Wooper, Quagsire, Goldeen, Shuckle,
	// Smeargle and Pineco as unavailable, and the oracle calls the tables
	// that still carry them (Safari Zone Southeast, Altering Cave, Artisan
	// Cave) unreachable content; rolling them caught a Pokemon the game does
	// not have, or — with no learnset to give it moves — refused the catch
	// and spent the encounter. Contested species (Geodude, an open question)
	// stay rollable until the question is settled.
	const real = mon => {
		if (!profile.oracle.availabilityOfSpecies) return true;
		const status = profile.oracle.availabilityOfSpecies(mon.species).status;
		return status !== 'unavailable' && status !== 'unreachable';
	};
	const pool = found.mons.filter(mon => mon.method === method && !mon.dupe && real(mon));
	if (!pool.length) {
		const removed = found.mons.some(mon => mon.method === method && !real(mon));
		throw new Error(`roll: everything ${method} turns up on ${found.name} is ` +
			(removed ? 'a dupe or a species this hack removed' : 'a dupe') + ' — roll another method or another route');
	}
...
/**
 * What the open marts sell, and whether this run already bought each row.
 * One purchase per shop row: nothing models money, so the bound is what
 * keeps a free stone from becoming an infinite one. A placed acquire
 * (item + the row's location) is the receipt.
 */
function shopItems(run) {
	const profile = getProfile(run.profileId);
	if (!profile.oracle.shopItems) return [];
	const bought = new Set();
	for (const entry of run.log) {
		if (!entry.command || entry.command.kind !== 'acquire') continue;
		if (!entry.command.where) continue;
		bought.add(entry.command.item + '|' + normPlace(entry.command.where));
	}
	return profile.oracle.shopItems()
		.filter(item => anchorOpen(run, item.opensAt))
		.map(item => ({
			name: item.name,
			kind: item.kind,
			location: item.location,
			opensAt: item.opensAt,
			bought: bought.has(item.name + '|' + normPlace(item.location)),
		}));
}
...
function fieldItems(run, map) {
	...
	// Placed acquires cross off their own instance; unplaced ones (every
	// acquire before `where` existed, and hand-recorded pickups) drain a
	// per-name pool in list order, which is exactly the old behavior.
	const placed = {};
	const unplaced = {};
	for (const entry of run.log) {
		if (!entry.command || entry.command.kind !== 'acquire') continue;
		const n = entry.command.count || 1;
		if (entry.command.where) {
			const key = entry.command.item + '|' + normPlace(entry.command.where);
			placed[key] = (placed[key] || 0) + n;
		} else {
			unplaced[entry.command.item] = (unplaced[entry.command.item] || 0) + n;
		}
	}
	const seenPlaced = {};
	return profile.oracle.fieldItems()
		...
		.map(item => {
			const key = item.name + '|' + normPlace(item.location);
			seenPlaced[key] = seenPlaced[key] || 0;
			let collected = (placed[key] || 0) > seenPlaced[key];
			seenPlaced[key] += 1;
			if (!collected && (unplaced[item.name] || 0) > 0) {
...
function preFightOpportunities(run) {
	...
	const empty = {
		...
		// Whether the party can answer a sash-threshold set in the NEXT fight.
		// Lilith's sash Mankey swept six slower Pokemon from 2% HP because
		// nothing alive was faster and nothing carried priority: a priority
		// move kills a 1-HP survivor before it moves, whatever the speeds. So
		// when the next fight holds such a set, this names the party's priority
		// answers — and when there are none, the priority moves someone could
		// be taught before walking in.
		thresholdPrep: {threats: [], priorityAnswers: [], teachable: [], covered: true},
		// The grind between a catch and the party the plan is drawn for. Every
		// other key here is something to GO AND GET; this is the one that is
		// already yours and not yet earned, and it was the only kind of work the
		// list did not name. The projection to the cap is correct and stays
		// correct — a plan drawn at current levels would be a plan for a team
		// nobody fields — so the silence was never about the numbers being
		// wrong. It was that a run sitting at catch levels reads 'Turtwig L12 ·
		// at cap' and is told nothing about the seven levels between.
		levels: {cap: null, setBy: null, behind: [], levelsNeeded: 0, note: 'No level cap applies.'},
	};
	if (!next) return empty;

	// Named against the fight being prepared for, not against the run's
	// current cap: `capAt` answers for one order, and the two differ exactly
	// when the next fight is the one that raises it.
	const capHere = capAt(run, next.order);
	if (typeof capHere === 'number') {
		const milestone = levelCap(run);
		// Only members BELOW it. The projection raises to the cap and never
		// lowers, so an over-cap Pokemon is not work owed — it is already past
		// the line and fights at its own level.
		const behind = run.party
			.map(id => findMon(run, id))
			.filter(mon => mon && mon.level < capHere)
			.map(mon => ({
				id: mon.id, species: mon.species, nickname: mon.nickname || null,
				level: mon.level, to: capHere, gain: capHere - mon.level,
			}));
		const levelsNeeded = behind.reduce((total, row) => total + row.gain, 0);
		empty.levels = {
			cap: capHere,
			setBy: milestone && milestone.trainer ?
				{trainer: milestone.trainer, ace: milestone.ace || null, tier: milestone.tier || null} :
				null,
			behind,
			levelsNeeded,
			note: levelsNeeded ?
				`${behind.length} of ${run.party.length} below the cap of ${capHere}, ` +
					`${levelsNeeded} level${levelsNeeded === 1 ? '' : 's'} between them` :
				`the whole party is at the cap of ${capHere}`,
		};
	}

	const nextFight = require('./planner').getFight(next.trainer, run.profileId);
	const threats = thresholdThreatsIn(nextFight);
	if (threats.length) {
		const ai = require('../ai');
		// A damaging move with priority. Status priority (Protect at +4) does
		// not kill a 1-HP survivor, which is the whole job.
		const attackPriority = name => {
			const meta = ai.getMoveMetadata(name, 8);
			return meta && meta.category !== 'Status' ? (meta.priority || 0) : 0;
		};
		...
		for (const id of run.party) {
			...
			for (const move of mon.moves || []) {
				if (attackPriority(move) > 0) {
					priorityAnswers.push({id, species: mon.species, move});
				}
			}
		}
		// Only shop for a fix when the party lacks one: the point is a missing
		// tool, not a catalogue.
		const teachable = [];
		if (!priorityAnswers.length) {
			for (const id of run.party) {
				const mon = findMon(run, id);
				if (!mon) continue;
				let options;
				try {
					options = learnable(run, id, {atLevel: mon.level});
				} catch (error) { continue; }
				for (const row of options.now || []) {
					if (attackPriority(row.move) > 0) {
						teachable.push({id, species: mon.species, move: row.move});
					}
				}
			}
		}
		empty.thresholdPrep = {
			...
			priorityAnswers,
			teachable,
			covered: priorityAnswers.length > 0,
		};
	}
	...
	if (profile.oracle.moveAvailability) {
		...
		// The run-aware cut: which of those move locations the run can walk
		// to RIGHT NOW. Undated rows are never counted reachable — their
		// locations render as leads, not promises.
		if (empty.moves.status === 'dated') {
			const reachable = empty.moves.available.filter(row =>
				row.opensAt !== null && anchorOpen(run, row.opensAt));
			const undated = empty.moves.available.filter(row => row.opensAt === null).length;
			empty.moves = {
				status: 'dated',
				count: reachable.length,
				reachable,
				undated,
				note: reachable.length + ' reachable now' +
					(undated ? ' · ' + undated + ' more with known places, undated' : ''),
			};
		}
	}
	return empty;
...
/**
 * What a Pokemon evolves into and whether the run is already there. The
 * evolutions ledger is ROM truth; `ready` is the only run-aware bit.
 */
function evolutionReadiness(run, id) {
	const mon = requireMon(run, id);
	const profile = getProfile(run.profileId);
	const rows = profile.oracle.evolutionsOf ? profile.oracle.evolutionsOf(mon.species) || [] : [];
	return {
		species: mon.species,
		level: mon.level,
		evolutions: rows.map(row => ({
			...
			ready: row.method === 'level' ? mon.level >= row.level : null,
		})),
	};
}
...
const COMMANDS = {
	...
	unhold(run, command) {
		...
		// The Game Corner pays out ONCE a run, from any tier whose gym is
		// beaten, random within the tier (operator ruling 2026-09-20, who has
		// played it; the workbook says only that each badge "unlocks a new
		// set"). The harness claimed one per badge until then — up to eight
		// extra bodies and a mythical — so the rule lives here, where no
		// caller can forget it.
		if (command.prize) {
			const tiers = profile.oracle.prizeTiers ? profile.oracle.prizeTiers() : [];
			const tier = tiers.find(entry => entry.badge === command.prize);
			if (!tier) throw new Error(`catch: ${command.prize} is not a Game Corner tier`);
			if (!tier.options.includes(command.species)) {
				throw new Error(`catch: the ${tier.badge} tier pays ${tier.options.join(', ')}, ` +
					`not ${command.species}`);
			}
			if (run.position < tier.opensAt) {
				throw new Error(`catch: the ${tier.badge} tier opens when ${tier.leader} is beaten`);
			}
			const prior = (run.log || []).find(entry => entry.command.kind === 'catch' &&
				entry.command.prize);
			if (prior) {
				throw new Error('catch: the Game Corner pays out once a run, and this run took ' +
					`${prior.command.species} from the ${prior.command.prize} tier`);
			}
		}
		...
		run.nextId += 1;
		run.box.push(mon);
		return `caught ${noteName(run, mon)} at level ${level}` +
	...
	identify(run, command) {
		...
		return `recorded ${noteName(run, mon)}: ${changed.join(', ')}`;
	},
	...
	levelUp(run, command) {
		...
		mon.level = to;
		// Levelling TEACHES. This raised the number and left the moveset
		// alone, so a Chimchar taken from 5 to 12 still knew only its level-5
		// moves and never learned Mach Punch — every fight after that was
		// fought with a moveset the real Pokemon would not have.
		//
		// A free slot fills automatically, because that is what the game does
		// without asking. A FULL moveset is left alone: the game stops and
		// asks which move to forget, and only the player knows the answer, so
		// those are reported for a deliberate `teach` rather than guessed.
		const learned = projectMoves(getProfile(run.profileId), {
			species: mon.species, level: from, moves: mon.moves,
		}, to);
		mon.moves = learned.moves;
		// The game's own prompt: a move the level-up offered while the moveset
		// was full is free to take over something, right then. Taking it LATER
		// is the nurse's "Remember a move", which charges a Heart Scale
		// (operator, from the second nurse's menu, 2026-09-20).
		if (learned.pending.length) mon.prompted = learned.pending.slice();
		else delete mon.prompted;
		return `${noteName(run, mon)} grew from L${from} to L${to}` +
			(spent ? ` (${spent} Rare Candy over the cap)` : '') +
			(learned.taught.length ? ` and learned ${learned.taught.join(', ')}` : '') +
			(learned.pending.length ?
				`; its moves are full, so ${learned.pending.join(', ')} ` +
				'must be taught over something' : '');
	},
	...
	evolve(run, command) {
		...
		// A stat-conditioned branch is this Pokemon's, not the player's choice:
		// Tyrogue becomes whichever Hitmon its Attack and Defense decide.
		const dossierRules = require('./dossier');
		const allowed = step => dossierRules.meetsRequirement(step, mon.species, mon.level, mon.ivs, mon.nature);
		const conditioned = options.filter(step => step.requires && step.requires.indexOf('def') !== -1);
		const target = command.into ?
			options.find(step => step.into === command.into) :
			options.length === 1 ? options[0] :
				conditioned.length === options.length ? options.find(allowed) || null : null;
		if (target && target.requires && !allowed(target)) {
			const own = options.find(allowed);
			throw new Error(`evolve: this ${mon.species}'s Attack and Defense make it a ` +
				`${own ? own.into : 'different form'}, not ${target.into} (${target.requires})`);
		}
		if (!target) {
		...
		// The ability travels by SLOT, not by name — which is what the games do
		// and what `the-lead-pulls-the-grass` already commits this run to. A
		// Horsea rolled into Swift Swim is Seadra's Poison Point afterwards,
		// because both sit in slot one; carrying the string across instead
		// leaves a Seadra holding an ability Seadra cannot have.
		//
		// That is not cosmetic. The provider validates the ability it is handed
		// against the species, so a stale one throws and the run loses its
		// survival forecast for that fight — 527 of the forecast failures on
		// record are exactly this, on Seadra, Fletchinder, Donphan, Staravia,
		// Herdier and Octillery.
		//
		// Slot is found by looking the old ability up in the OLD species' list,
		// so an unrecorded or unrecognised ability leaves the slot at zero
		// rather than inventing one.
		//
		// Today that indexing never does anything: every one of the 1,244
		// species in this fork's calc data carries exactly one ability, so the
		// slot is always zero and this is indistinguishable from "take the new
		// species' ability". It is written by slot anyway because that is the
		// rule, and because the day a second ability lands in the data is not
		// the day anyone will remember this line. The gate cannot falsify the
		// indexing for the same reason, and says so.
		if (mon.ability) {
			const before = abilityList(from);
			const after = abilityList(target.into);
			if (after.length) {
				const slot = Math.max(0, before.indexOf(mon.ability));
				mon.ability = after[slot] !== undefined ? after[slot] : after[0];
			}
		}
		return `${from} evolved into ${target.into}` +
	...
	teach(run, command) {
		...
		// The nurse's menu (operator, from the game, 2026-09-20): "Remember a
		// move" covers an egg move AND a move from a previous learnset, and
		// each costs one Heart Scale. Only the level-up's own prompt is free,
		// and only while it stands — mon.prompted, set by levelUp.
		const prompted = (mon.prompted || []).includes(command.move);
		const remembered = !prompted && verdict.sources.some(source =>
			isEggSource(source) || (source.level !== undefined && source.level <= mon.level));
		const relearnerOnly = remembered;
		if (relearnerOnly) {
			const held = run.bag[HEART_SCALE] || 0;
			if (!held) {
				throw new Error(`teach: ${mon.species} must REMEMBER ${command.move} — ` +
					'an egg move or one from a previous learnset, and the nurse charges one ' +
					`Heart Scale; the bag has ${held}`);
			}
			run.bag[HEART_SCALE] = held - 1;
			if (!run.bag[HEART_SCALE]) delete run.bag[HEART_SCALE];
		}
		// A TM is a ONE-TIME item here. A move whose only free route is the
		// teachable pool comes from a TM or a tutor: a tutor is a service and
		// stays free, a TM must be in the bag and is spent unless it is an HM
		// or one of the ten the Lilycove Department Store re-sells. Without
		// this the run taught Rock Blast 42 times from a TM it never owned.
		const teachableOnly = !relearnerOnly && verdict.sources.every(source =>
			source.source === 'teachable' || (source.level !== undefined && source.level > mon.level));
		const tutored = teachableOnly && profile.oracle.tutorTeaches &&
			profile.oracle.tutorTeaches(command.move);
		// A tutor is free, but it stands somewhere, and the road has to have
		// reached it. `+ 1` as for a catch: position is the last fight BEATEN,
		// so what opens at the next order is already open.
		if (tutored && profile.oracle.tutorOpensAt) {
			const opens = profile.oracle.tutorOpensAt(command.move);
			if (typeof opens === 'number' && run.position + 1 < opens) {
				throw new Error(`teach: the tutor for ${command.move} is not reached until order ${opens}, ` +
					`and the run is at ${run.position}`);
			}
		}
		const tm = teachableOnly && !tutored && profile.oracle.tmFor ?
			profile.oracle.tmFor(command.move) : null;
		let tmSpent = null;
		if (tm) {
			const owned = run.bag[tm.name] || 0;
			// Re-sold at Lilycove means unlimited once Lilycove can be shopped
			// at — before that the copy in the bag is the only one there is.
			const unlimited = tm.repeatable && (tm.unlimitedFrom === undefined ||
				(tm.unlimitedFrom !== null && run.position >= tm.unlimitedFrom));
			// And once it IS on the shelf it need not be in the bag: money is
			// not modelled (ruling money-and-balls-are-not-modelled), so a TM
			// the store sells is one the player can always have. The first cut
			// of this rule still asked for a copy, and refused ten teaches of
			// Icy Wind in the first run to finish the game — its one copy had
			// been spent twenty orders before Lilycove opened. An HM is never
			// sold, so it must be held.
			const onTheShelf = unlimited && !/^HM/.test(tm.name);
			if (!owned && !onTheShelf) {
				throw new Error(`teach: ${command.move} comes from ${tm.name}, and the bag has none`);
			}
			if (!unlimited) {
				run.bag[tm.name] = owned - 1;
				if (!run.bag[tm.name]) delete run.bag[tm.name];
				tmSpent = tm.name;
			}
		}
		if (prompted) {
			mon.prompted = (mon.prompted || []).filter(move => move !== command.move);
			if (!mon.prompted.length) delete mon.prompted;
		}
		const price = relearnerOnly ? ', for one Heart Scale' :
			prompted ? ', from the level-up prompt' :
				tmSpent ? ', spending ' + tmSpent : '';
		if (mon.moves.length >= 4) {
			...
			return `${noteName(run, mon)} forgot ${command.replace}, learned ${command.move}${price}`;
		}
		...
		return `${noteName(run, mon)} learned ${command.move}${price}`;
	},
	...
	give(run, command) {
		...
		// Most of a bag is not a held item — Potions, Rare Candy, Heart Scales,
		// repels. Accepting one stored a Pokemon the calculator refuses to
		// build, and the refusal arrived later, from planner.playerStateFromEntry,
		// naming a battle slot rather than a box entry: the run could not plan
		// or fight until somebody worked out who was holding what. The rule was
		// already here and already used one function away, where the advisor
		// filters its own give options.
		if (!require('./planner').holdableItem(command.item)) {
			throw new Error(`give: ${command.item} is not an item a Pokemon can hold — ` +
				'it stays in the bag');
		}
		if (mon.item) {
		...
		return `${noteName(run, mon)} is holding ${command.item}` +
	...
	take(run, command) {
		...
		const item = mon.item;
		mon.item = null;
		return `took ${item} from ${noteName(run, mon)}`;
	},

	/**
	 * A held item was used up or removed in a fight: an Oran Berry eaten, a
	 * Focus Sash spent, a Gem fired, a Leftovers knocked off. It is gone —
	 * not back to the bag. Run & Bun's own rule (Mechanic Changes, "Held
	 * items"): items consumed or removed during battle are not restored.
	 *
	 * A fight outcome like `faint`, and recorded the same way — by whoever
	 * played the fight, which is the only party that knows the berry fired.
	 * `beat` cannot infer it: a Chople Berry eats only if a Fighting hit
	 * lands. Win or loss alike, so it is its own command, not a field of
	 * `beat`. The item is named so a replay that drifted (the holder no
	 * longer holds it) refuses instead of eating something else.
	 */
	consume(run, command) {
		const mon = requireMon(run, command.id);
		if (!command.item) throw new Error('consume: item is required');
		if (mon.item !== command.item) {
			throw new Error(`consume: ${mon.species} (${mon.id}) is holding ` +
				`${mon.item || 'nothing'}, not ${command.item}`);
		}
		mon.item = null;
		return `${noteName(run, mon)}'s ${command.item} was used up in the fight` +
			(command.to ? ` against ${command.to}` : '') + ' — Run & Bun does not restore it';
	},
	...
	heartScale(run, command) {
		const mon = requireMon(run, command.id);
		// CHANGE NATURE, the nurse's other scale service: three Heart Scales
		// (ruling remembering-a-move-costs-a-scale, from the operator's own
		// screenshot of the menu). It was ruled and never modelled — the run
		// could max an IV and nothing else — so no run had ever changed a
		// nature, and a box arrived at every wall with whatever it rolled:
		// a Rash Kingdra with a Sp. Def IV of 6 took 77% from a Tri Attack.
		if (command.nature !== undefined) {
			const nature = String(command.nature).trim();
			if (!NATURES.has(nature)) throw new Error(`heartScale: ${JSON.stringify(command.nature)} is not a nature`);
			if (mon.nature === nature) {
				throw new Error(`heartScale: ${mon.species} (${mon.id}) is already ${nature}; ` +
					'three Heart Scales would buy nothing');
			}
			const bag = run.bag[HEART_SCALE] || 0;
			if (bag < NATURE_SCALES) {
				throw new Error(`heartScale: changing a nature costs ${NATURE_SCALES} Heart Scales — ` +
					`need ${NATURE_SCALES}, the bag has ${bag}` + whereToFind(run, HEART_SCALE));
			}
			run.bag[HEART_SCALE] = bag - NATURE_SCALES;
			if (!run.bag[HEART_SCALE]) delete run.bag[HEART_SCALE];
			const was = mon.nature || 'unrecorded';
			mon.nature = nature;
			return `${noteName(run, mon)} nature ${was} → ${nature} ` +
				`(${NATURE_SCALES} Heart Scales spent, ${bag - NATURE_SCALES} left)`;
		}
		...
		if (!held) {
			throw new Error('heartScale: one Heart Scale sets one IV to 31 and no shop ' +
				...
				whereToFind(run, HEART_SCALE));
		}
		run.bag[HEART_SCALE] = held - 1;
		if (!run.bag[HEART_SCALE]) delete run.bag[HEART_SCALE];
		mon.ivs[command.stat] = 31;
		return `${noteName(run, mon)} ${IV_STATS[command.stat]} IV ` +
	...
	acquire(run, command) {
		...
		// WHERE is optional and matters for one reason: thirty Heart Scales
		// share one name, and crossing off a field pickup by name alone
		// marked every sibling collected. A placed acquire crosses off only
		// its own instance; an unplaced one keeps the old name-pool behavior.
		if (command.where !== undefined && command.where !== null) {
			if (typeof command.where !== 'string' || !command.where.trim() ||
				command.where.length > 120) {
				throw new Error('acquire: where must be a place name up to 120 characters');
			}
		}
	...
	party(run, command) {
		...
		run.party = ids.slice();
		const names = ids.map(id => noteName(run, findMon(run, id)));
		return names.length === 0 ? 'the party is empty' :
			names.length === 1 ? `${names[0]} takes the lead` :
				`${names[0]} takes the lead — with ${names.slice(1).join(', ')}`;
	},
	...
	skip(run, command) {
		...
		// A DOUBLES fight is delayable by class: the play stack cannot run
		// two actives yet (operator ruling 2026-08-28: doubles are real
		// doubles, skip them for the time being), so the road may go around
		// any of them the way it goes around Camper Gavi — owed, not waived.
	...
	faint(run, command) {
		...
		if (!run.rules.permadeath) {
			return `${noteName(run, mon)} fainted; permadeath is off, so nothing changed`;
		}
 ...
 *
 * `exposureWeight` (default EXPOSURE_WEIGHT) charges the six that many points for
 * every member an enemy one-shots from the slower side, summed over enemies.
 * The best-answer sum cannot see a six that shares a weakness: three bodies
 * the same enemy kills before they move cost nothing past the one that
 * answers it. Among the ranker's own top six sixes on the 87 held-out fights,
 * the set score tracked selection wins at r = 0.006 and this count at -0.17
 * (2026-09-18). As a term it gained +80 seeds on those fights and +54 on
 * 80 it was not chosen on (heldout3), and +40 on top of pick-by-play; 0
 * is the ranker as it stood before.
 *
...
function answerTable(matrix) {
	...
	return matrix.box.map((member, m) => enemies.map((enemy, e) => {
		...
		return {
			...
			// The enemy one-shots this member before it can act: read by the
			// ranker's optional exposure term, not by the answer arithmetic.
			exposed: !!cell.them.guaranteedKO && cell.speed !== 'faster',
		};
	}));
}

/**
 * The cheap cut: which box members could ever win an enemy column.
 *
 * `scoreSix` is `sum over enemies of (best member in the six for that column)`,
 * so a six is worth exactly what its COLUMN WINNERS are worth. A member that
 * never wins a column contributes nothing to the sum — swapping it for any
 * other member leaves the score untouched, which the adjudication comment below
 * already relies on when it picks candidates for distance.
 *
 * So keep, per column, the `keepPerColumn` best members, and the same for the
 * lead-free reading of the enemy-lead column. A member outside every one of
 * those lists can only win a column in a six that excludes all `keepPerColumn`
 * members above it — a six that is, by construction, weak in that column.
 *
 * This is a HEURISTIC, not a proof, and it is not asserted as one. Measured
 * against exhaustive over 27 fight/level pairs spread across the whole run,
 * box of 37, comparing the pruned ranking to the full 2,324,784-combination
 * one:
 *
 *   keepPerColumn   best six's score   whole top-10 vector   mean combinations
 *              4            27 of 27             19 of 27               1,856
 *              6            27 of 27             24 of 27               9,501
 *              8            27 of 27             25 of 27              34,449
 *             12            27 of 27             25 of 27             162,430
 *
 * The BEST SCORE is never wrong, at any setting tried, including four. What the
 * cut costs is depth: the tail of the top ten sometimes holds a different six
 * of equal or near-equal score, because the pool cannot offer every tied
 * alternate. That plateaus at eight — twelve buys nothing over eight — so the
 * default of six sits where the curve turns, at 245x less work.
 *
 * The sixes themselves differ from exhaustive almost every time even when the
 * scores match exactly, and that is the tiebreak rather than the ranking: many
 * sixes score identically and the winner among them is the lexicographically
 * first, which changes when the pool changes. A gate in tests/run.test.js
 * compares pruned against exhaustive and fails if the best SCORE ever moves.
 *
 * `exhaustive: true` turns the cut off for the creative case — looking for a
 * sacrifice or an off-read is exactly when a member that answers nothing is
 * the point — and still respects the combination ceiling, so the cost of the
 * whole box is something a caller accepts rather than discovers.
 */
function shortlistMembers(answers, enemyCount, keepPerColumn) {
	const keep = new Set();
	const all = answers.map((unused, m) => m);
	const takeBest = read => {
		const ordered = all.slice().sort((a, b) => read(b) - read(a) || a - b);
		for (const m of ordered.slice(0, keepPerColumn)) keep.add(m);
	};
	for (let e = 0; e < enemyCount; e++) takeBest(m => answers[m][e].withEntry);
	// The enemy-lead column is read twice — once entry-priced, once lead-free —
	// because the lead marginalisation scores it both ways and a member can be
	// competitive under one reading and not the other.
	takeBest(m => answers[m][0].leadFree);
	return Array.from(keep).sort((a, b) => a - b);
}

/**
 * The order of two played sixes: wins, then expected deaths, then the grid
 * score; with `survival`, expected deaths first. At three frontier fights
 * (Michelle, Cranberry, Pablo; 2026-09-23, six rollouts) the two orders were
 * identical: more wins and fewer deaths went together, a lost rollout counting
 * everyone fielded. They part only where a six wins more by paying in wins.
 */
function playedOrder(a, b, survival) {
	return survival ?
		a.adjudication.eDeaths - b.adjudication.eDeaths ||
		b.adjudication.pWin - a.adjudication.pWin || b.score - a.score :
		b.adjudication.pWin - a.adjudication.pWin ||
		a.adjudication.eDeaths - b.adjudication.eDeaths || b.score - a.score;
}
...
function rankParties(run, trainer, options) {
	...
	// ONE MEGA A SIX, CHOSEN (ruling the-player-megas; the limit it recorded).
	...
	// which can field only one. Seed 418957's box holds ELEVEN such bodies, so
	// a six of them was priced as a team of Megas and played as one Mega and
	// five base forms. The base board is built beside it and a six is scored
	// with at most one Mega: whichever single body is worth the most as one.
	// Built only when a Mega is actually in play, because it costs a second
	// matchup grid and three-quarters of sixes hold no Mega at all.
	const megaRows = members.map((member, m) => (member.mega ? m : -1)).filter(m => m >= 0);
	const baseAnswers = megaRows.length ?
		answerTable(boxMatrix(run, trainer, {megas: 'none'})) : answers;

	// The dossier's named answers, as a TIE-BREAK only: one point in the
	// thousandths per named body — at most 0.006 for a full six, so it can
	// tip near-ties toward the key species but not overturn any difference
	// the board states in hundredths (the comparator itself was A/B'd to a
	// draw and ships on reasoning, so it is not re-litigated here).
	const dossierProfile = getProfile(run.profileId);
	const dossierRow = dossierProfile.oracle && dossierProfile.oracle.fightDossierOf ?
		dossierProfile.oracle.fightDossierOf(matrix.trainer) : null;
	const namedAnswers = new Set(dossierRow ? [
		...(dossierRow.keyBox || []),
		...(dossierRow.mons || []).flatMap(mon => mon.topAnswers || []),
	] : []);
	const keyMembers = new Set();
	if (namedAnswers.size) {
		const evolveTo = require('./dossier').evolveTo;
		members.forEach((member, m) => {
			const owned = run.box.find(entry => entry.id === member.id);
			const fielded = owned ? require('./dossier').evolveMon(owned, member.level) :
				evolveTo(member.species, member.level);
			if (namedAnswers.has(fielded)) keyMembers.add(m);
		});
	}

	// The enumeration below is exhaustive over its pool and has no natural
	// stopping point, so the cost is decided BEFORE the loop rather than
	// sampled inside it. Every box gets the cheap cut; only a caller that asks
	// for `exhaustive` pays for the whole box.
	...
	const keepPerColumn = opts.keepPerColumn === undefined ?
		PARTY_LIMIT : opts.keepPerColumn;
	const pruneAbove = opts.pruneAbove === undefined ?
		RANK_PRUNE_ABOVE : opts.pruneAbove;
	const wholeBox = members.map((unused, m) => m);
	const cutting = !opts.exhaustive &&
		countCombinations(members.length, Math.min(PARTY_LIMIT, members.length)) > pruneAbove;
	// The star is in the pool by construction, not by luck. It is the member
	// with the best total across every column, and the `without the star` row
	// below is defined by excluding it — a star the cut had dropped would make
	// that row the overall best six under a different name.
	const pool = cutting ?
		Array.from(new Set([star,
			...shortlistMembers(answers, enemies.length, keepPerColumn)]))
			.sort((a, b) => a - b) :
		wholeBox;
	const size = Math.min(PARTY_LIMIT, pool.length);
	const limit = opts.maxCombinations === undefined ?
		RANK_COMBINATION_LIMIT : opts.maxCombinations;
	const combinations = countCombinations(pool.length, size);
	if (combinations > limit) {
		throw new Error(
			`rank: ${pool.length} candidates is ` +
			`${combinations.toLocaleString('en-US')} possible sixes, over the ` +
			`${limit.toLocaleString('en-US')} this ranks exhaustively. ` +
			(opts.exhaustive ?
				'Drop `exhaustive`, or raise maxCombinations to accept the cost.' :
				'Lower keepPerColumn, or raise maxCombinations to accept the cost.'));
	}

	const exposureWeight = opts.exposureWeight === undefined ? EXPOSURE_WEIGHT : opts.exposureWeight;
	if (typeof exposureWeight !== 'number' || !(exposureWeight >= 0)) {
		throw new Error('rank: exposureWeight is a number of points at least 0, not ' +
			JSON.stringify(exposureWeight));
	}
	const exposureOf = answers.map(row => row.filter(cell => cell.exposed).length);

	const collapseThreshold = 0.75;
	/** Score a six with exactly one of `picks` fielded as its Mega (or none). */
	function scoreWithMega(picks, megaPick) {
		const rowOf = m => (megaRows.length === 0 || m === megaPick ? answers[m] : baseAnswers[m]);
		...
		for (let e = 1; e < enemies.length; e++) {
			let top = -Infinity;
			for (const m of picks) top = Math.max(top, rowOf(m)[e].withEntry);
			tailSum += top;
		...
		for (const lead of picks) {
			...
			for (const m of picks) {
				leadColumn = Math.max(leadColumn,
					m === lead ? rowOf(m)[0].leadFree : rowOf(m)[0].withEntry);
			}
		...
		let keyBonus = 0;
		for (const m of picks) if (keyMembers.has(m)) keyBonus += 0.001;
		let exposure = 0;
		if (exposureWeight) for (const m of picks) exposure += exposureOf[m];
		return {
			score: round(best.total + keyBonus - exposureWeight * exposure),
			lead: best.lead,
	...
	/**
	 * The six, with the Mega it should field — the choice made WITH the six
	 * rather than after it. A body only megas if being a Mega earns the set
	 * more than it costs the others nothing, so a six of Mega-capable bodies
	 * is priced as what it can actually field.
	 */
	function scoreSix(picks) {
		const candidates = megaRows.filter(m => picks.includes(m));
		if (!candidates.length) return Object.assign(scoreWithMega(picks, -1), {mega: null});
		let best = null;
		for (const megaPick of candidates.concat([-1])) {
			const scored = scoreWithMega(picks, megaPick);
			if (best === null || scored.score > best.scored.score) best = {megaPick, scored};
		}
		return Object.assign(best.scored,
			{mega: best.megaPick === -1 ? null : {id: members[best.megaPick].id, species: members[best.megaPick].mega}});
	}
	...
	// `cursor` walks positions in the candidate pool; `picks` is the member
	// indices those positions name. Everything downstream — answers, members,
	// star — is indexed by MEMBER, so the mapping happens here and nowhere else.
	const cursor = Array.from({length: size}, (unused, i) => i);
	const picks = cursor.map(c => pool[c]);
	...
	for (;;) {
		...
		// Next combination in lexicographic order, over pool POSITIONS.
		let i = size - 1;
		while (i >= 0 && cursor[i] === pool.length - size + i) i--;
		if (i < 0) break;
		cursor[i]++;
		for (let j = i + 1; j < size; j++) cursor[j] = cursor[j - 1] + 1;
		for (let j = 0; j < size; j++) picks[j] = pool[cursor[j]];
	}
	...
	function present(scored, label) {
		...
		// Read through the six's OWN Mega choice: the body that megas answers as
		// its Mega, every other body as itself.
		const megaId = scored.mega ? scored.mega.id : null;
		const rowOf = m => (megaRows.length === 0 || members[m].id === megaId ? answers[m] : baseAnswers[m]);
		const perEnemy = enemies.map((enemy, e) => {
			let bestMember = null;
			let bestValue = -Infinity;
			for (const m of scored.picks) {
				const value = e === 0 && m === scored.lead ?
					rowOf(m)[0].leadFree : rowOf(m)[e].withEntry;
		...
		return {
			...
			// The ONE body this six fields as a Mega, or null: chosen with the
			// six, not after it, so the score is a score of what it can field.
			mega: scored.mega || null,
			perEnemy,
		};
	}
	...
	// Adjudication plays singles rollouts (driver.start refuses a double), so
	// a double battle is ranked by the grid alone. Before this, ranking a
	// double threw, and every caller that re-picks a six crashed on one or —
	// the headless harness — fell back to a level sort without saying so.
	const isDouble = !!require('./planner').getFight(matrix.trainer, run.profileId).isDouble;
	if (rollouts > 0 && parties.length && !isDouble) {
		const driver = require('./battle-driver');
		const adjudicated = Math.min(opts.adjudicate === undefined ? 4 : opts.adjudicate,
			parties.length);
		// Play four DIFFERENT sixes, not four spellings of one.
		//
		// scoreSix is the best answer per enemy column, so swapping the member
		// that answers nothing changes no column and no score. The shortlist
		// that came out of it was systematically near-duplicates: measured over
		// twelve fights, the top four always shared five of six members and
		// carried one identical grid score, every one of the four adjudications
		// returned the same pWin and the same eDeaths, and playing them
		// reordered the top six zero times. Thirty-six of the forty-eight
		// rollouts were replaying the same team.
		//
		// So the candidates are picked for DISTANCE as well as rank: the best
		// six, then the best that differs from every chosen one by at least two
		// members. Same budget, four real opinions — and when a box cannot
		// supply four distinct sixes, fewer rollouts rather than repeated ones.
		const chosen = [];
		for (let i = 0; i < parties.length && chosen.length < adjudicated; i++) {
			const ids = new Set(parties[i].members.map(member => member.id));
			const distinct = chosen.every(index => {
				const other = parties[index].members
					.filter(member => ids.has(member.id)).length;
				return ids.size - other >= 2;
			});
			if (!chosen.length || distinct) chosen.push(i);
		}
		...
		const played = chosen.map(index => parties[index]);
		// Under permadeath (opts.survival) the order is the other way round:
		// expected bodies lost first — a lost rollout loses everyone fielded, so
		// it already carries the loss — then wins. A six that wins 11 of 12 by
		// trading two bodies each time is worse than one that wins 10 losing none.
		played.sort((a, b) => playedOrder(a, b, opts.survival));
		// The played sixes come first whatever their grid position was; the
		// rest keep grid order behind them.
		const unplayed = parties.filter(party => !party.adjudication);
		parties.length = 0;
		parties.push(...played, ...unplayed);
	...
	return {
		...
		order: matrix.order,
		fightNumber: trainerIndexOf(run, matrix.order),
		projection: matrix.projection,
		boxSize: members.length,
		combinations: top.length ? combinations : 0,
		setScore: {exposureWeight},
		// What the cut dropped, named rather than implied. `candidates` is the
		// pool actually enumerated and `cut` the members no enemy column ranked
		// highly enough to matter; a caller comparing `candidates` to `boxSize`
		// can see the reduction, and `exhaustive: true` makes them equal.
		shortlist: {
			candidates: pool.length,
			cut: members.length - pool.length,
			keepPerColumn: cutting ? keepPerColumn : null,
			// Not the same statement as `cut: 0`. A box under the threshold was
			// never cut; a box over it can still lose nobody.
			cutting,
			exhaustive: !!opts.exhaustive,
			dropped: wholeBox.filter(m => !pool.includes(m)).map(m => members[m].id),
		},
...
function fightPlaybook(run, trainer, options) {
	...
	//
	// How many rollouts each variant gets is the accuracy knob that mattered
	// most and was the only one a caller could not reach. Measured over eight
	// real mid-run states pulled from playthrough output, positions 73 to 282,
	// judging every variant on four rollouts against judging it on twelve:
	//
	//   - the two budgets chose a DIFFERENT variant in four of the eight
	//   - where they differed, twelve was better four times and worse zero
	//   - at Leader Brawly and Trainer Chelle Daycare twelve moved pWin from
	//     0.00 to 0.08 — it found a line in a fight four rollouts called a
	//     certain wipe
	//   - it costs 2.61x, which is why it is not the default
	//
	// Four stays the default because it is the cost the tool has always paid.
	// What changes is that `variantRollouts` now exists, so a player who wants
	// a more accurate plan can buy one instead of discovering that the rollouts
	// they asked for only reached the final line. Successive halving was tried
	// here and refused: judged over the same eight fights it matched the
	// twelve-rollout pick no more often than flat four did, went better once
	// and worse once, and cost 1.67x — two-rollout rounds discard good variants
	// on a signal whose resolution is one half.
	const rollouts = opts.rollouts === undefined ? 12 : opts.rollouts;
	const variantRollouts = opts.variantRollouts === undefined ?
		Math.min(4, rollouts) : Math.min(opts.variantRollouts, rollouts);
	...
	return {
		...
		assignments: chosen,
		explored,
		// `explored` alone reads as a count of cheap things. Each one is a
		// played fight, so the rollouts behind it belong beside it: the search
		// costs explored x variantRollouts playthroughs before the final line.
		variantRollouts: explored ? variantRollouts : 0,
		odds: played.odds,
...
function summarize(run) {
	...
	return {
		...
		split: split(run),
		next: ahead.length ? {trainer: ahead[0].trainer, order: ahead[0].order,
			fightNumber: trainerIndexOf(run, ahead[0].order)} : null,
...
module.exports = {
	VERSION, PARTY_LIMIT, EXPOSURE_WEIGHT, LEVEL_CAP_MODES, COMMANDS, IV_STATS, HEART_SCALE,
	createRun, answerTable, playedOrder, megaFormOf, megaRingHeld, stoneInBag, apply, applyAll, undo,
	findMon, levelCap, capAt, upcoming, eliteFourCounterpart, trainerIndexOf, shopItems, milestones, split, splitPrep, fightTier, isExcludedVariant,
	encountersOn, unusedRoutes, encounterRules, dupeKey, requireLayer, learnable, evolutionReadiness, partySpecs, planNext, boxMatrix, safetyPath, pairingLethal, affordableFromBag, spendFromBag, affordableOptions,
