 ...
 *
 * Deliberately conservative: a speed TIE is not outspeeding, and priority
 * moves, multi-hit survivals (Sturdy, Sash) and switch-ins are not credited
 * here — the pairing is scored as the one clean exchange it describes.
 *
...
function safetyPath(run, trainer) {
	...
	perMon.forEach(entry => {
		const affordable = entry.improving.filter(option =>
			affordableFromBag(optionItems(option), bagLedger));
		const pool = affordable.length ? affordable : entry.improving;
...
module.exports = {
	...
	encountersOn, unusedRoutes, encounterRules, requireLayer, learnable, evolutionReadiness, partySpecs, planNext, boxMatrix, safetyPath, pairingLethal, affordableFromBag, spendFromBag,
