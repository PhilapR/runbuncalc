...
function critExposure(grid, slotCount) {
	...
	for (let slot = 0; slot < slotCount; slot += 1) {
		const killers = grid
			.filter(cell => cell.versus[slot] && cell.versus[slot].them &&
				cell.versus[slot].them.critKO)
			.map(cell => ({
...
function safetyPath(run, trainer) {
	...
	run.party.forEach((id, slot) => {
		...
		for (const candidate of candidates.list) {
			...
			const stillLethal = row.filter(cell => cell.them && cell.them.critKO).length;
			const wasLethal = base.filter(cell => cell.them && cell.them.critKO).length;
			if (stillLethal >= wasLethal) continue;
			answer.steps.push({
				kind: candidate.kind,
				id,
				species: mon.species,
				detail: candidate.detail,
				removes: wasLethal - stillLethal,
				remaining: stillLethal,
				// What it spends: the run cannot un-teach a move slot or
				// un-hold an item it gave away elsewhere.
				cost: candidate.kind === 'give' ? 'the item leaves the bag' :
					candidate.kind === 'teach' ? 'a move slot' :
						candidate.kind === 'level' ? 'nothing — the cap is free' : 'unpriced',
			});
		}
	});
