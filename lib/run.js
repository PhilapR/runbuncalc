...
function safetyPath(run, trainer) {
	...
	perMon.forEach(entry => {
		const affordable = entry.improving.filter(option => {
			...
			return items.every(item => (bagLedger[item] || 0) > 0);
		});
		...
		best.steps.forEach(step => {
			if (step.kind === 'give' || step.kind === 'pickup') {
				const item = step.spec.item;
				if (bagLedger[item]) bagLedger[item] -= 1;
			}
		});
		answer.steps.push({
			id: entry.id,
...
module.exports = {
	...
	encountersOn, unusedRoutes, encounterRules, requireLayer, learnable, evolutionReadiness, partySpecs, planNext, boxMatrix, safetyPath,
