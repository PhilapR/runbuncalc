...
function learnable(run, id, options) {
	...
	for (const move of Object.keys(all)) {
		...
		if (!entry.scale && profile.oracle.tmFor && sources.every(s =>
			...
			const tutored = profile.oracle.moveTutors &&
				profile.oracle.moveTutors().some(row => row.move === move);
...
const COMMANDS = {
	...
	teach(run, command) {
		...
		const tutored = teachableOnly && profile.oracle.moveTutors &&
			profile.oracle.moveTutors().some(row => row.move === command.move);
