...
/**
 * The random stream a battle's opening draws from: Trace's pick between two
 * foes and a speed tie between two leads. Without one the engine fell back
 * to Math.random, so the same fight built twice could open two ways and the
 * planner answered the same question differently run to run. The stream is
 * derived from a key that names the opening (the fight, its mode), so an
 * opening is the same every time it is asked. A caller that holds its own
 * stream (a seeded battle) passes `opts.random` instead.
 */
function openingStream(key) {
	// FNV-1a over the key, then mulberry32 — the generator lib/battle-driver.js
	// seeds its fights with.
	let seed = 0x811c9dc5;
...
function loadRunMap(profileId) {
	...
	for (const fight of byTrainer.values()) {
		...
		// The '&' convention plus the profile's declared doubles-format list:
		// the game spells most pairs "And", and Leader Juan fields two of his
		// own — the label alone cannot see either.
		const declaredDoubles = (profile.encounters && profile.encounters.DOUBLES_FORMAT) || new Set();
		fight.isDouble = fight.trainer.includes('&') || declaredDoubles.has(fight.trainer);
		fights.push(fight);
	}
...
function playerStateFromEntry(bridge_, mon, id) {
	...
	const built = bridge_.rebuildZeroEvPokemon({
		...
		nature: mon.nature,
		gender: mon.gender,
		// Two different questions, and the data already tells them apart.
		//
		// A spec with NO ivs key is a PROSPECT — a Pokemon nobody has caught, so
		// nobody has rolled it. adviseCatches builds exactly that shape,
		// {species, level, moves}, and grading it at 31 asks "is this worth
		// catching at its best", which is the question a catch advisor is for.
		//
		// A spec with an ivs OBJECT is one the run OWNS. A Pokemon caught
		// without a rolled spread stores ivs {}, and filling those gaps with 31s
		// planned it flawless: at level 30 a Poochyena reads 70 HP and 35 Speed
		// against 65/30 for an average roll and 61/26 for the worst. Five points
		// of Speed over average, nine over the worst, and five points of Speed
		// flips turn order, which flips a survival verdict.
		//
		// Optimistic is the dangerous direction for something you already have
		// and are about to fight with. The rule this repo plans by is that their
		// crit is what a plan must survive and OUR FLOOR is what a plan may rely
		// on, so an unrecorded stat on an owned Pokemon takes the floor while
		// every recorded one is kept. The panel says the same thing in words:
		// "IV 84+ · 3 not recorded", never a total it cannot know.
		//
		// lib/battle-driver.js:586 answers this by refusing outright — "roll the
		// encounter once before fighting it" — which is right where a roll is
		// there to be made. Refusing here would take the forecast away from a
		// legacy or imported save rather than give it a conservative one.
		ivs: mon.ivs ?
			Object.assign({hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0}, mon.ivs) :
 ...
 * assumption, not data.
 *
 * `opts.random` is the stream the opening draws from (Trace's pick, a lead
 * speed tie). Without it the opening draws from a stream derived from the
 * fight and mode (openingStream), so the same fight always opens the same way.
 */
function buildFightState(options) {
	...
	// The leads' entry abilities fire as the battle opens: until 2026-09-22
	// they never did, so Drizzle Kyogre opened in a dry sky, Primal Kyogre
	// too, and no Intimidate lead ever cut an Attack.
	state = ai.applyLeadEntries(state, {random: typeof opts.random === 'function' ? opts.random :
		openingStream('fight:' + fight.trainer + ':' + (doubles ? 'doubles' : 'singles'))});
	return {
		fight,
		state,
		borrowed,
...
function buildWildState(options) {
	...
	ai.validateBattleState(state);
	state = ai.applyLeadEntries(state, {random: typeof opts.random === 'function' ? opts.random :
		openingStream('wild:' + wildMon.species + ':' + wildMon.level)});
	return {state};
}
 ...
 */
/**
 * The hardest a side can possibly hit: the maximum CRIT roll across every
 * damaging move, not the crit band of whichever move won on average. Worst
 * case is a property of the whole movepool, and planning that assumes the
 * enemy uses its average-best move on a crit turn understates the ceiling.
 */
function critCeiling(evaluations) {
	let ceiling = null;
	for (const evaluation of evaluations) {
		...
		// A move with no crit band (already a guaranteed crit, or crit-blocked)
		// reports its ordinary maximum: that IS its worst case.
		const max = damage.critMax !== undefined ? damage.critMax : damage.max;
		if (!ceiling || max > ceiling.max) {
			ceiling = {move: evaluation.action.moveName, max, targetHp: damage.targetHp,
				crit: damage.critMax !== undefined,
				// Priority beats Speed. A consumer that cancels a threat
				// because "we are faster" must know when that is irrelevant.
				priority: evaluation.facts && evaluation.facts.priority > 0,
			};
		}
	}
	return ceiling;
 ...
 */
/**
 * One direction of a cell. `withCritBand` is off for OUR side, on purpose.
 *
 * The doctrine below is not decoration: their crit is what a plan has to
 * survive, and our floor is what a plan may rely on. So the player-side crit
 * band answers a question nobody is allowed to ask, and a grep across `lib`,
 * `src/js`, `scripts` and `tests` finds zero reads of `us.critMax`,
 * `us.critKO`, `us.critMove` or `us.critPriority` against five reads of the
 * `them` side.
 *
 * Reading it is what COMPUTED it. The engine defers the band behind an
 * accessor now, so `critCeiling` touching `critMax` was the only reason a
 * second `Calc.Move` and a second `Calc.calculate` were built for every
 * damaging move we could throw.
 *
 * The fields are OMITTED rather than zeroed, which is the same choice
 * `skipThem` makes below and for the same reason. A plausible-looking number
 * here would be a quiet lie; an absent one makes `us.critMax / hp` come back
 * NaN, which is a caller finding out.
 */
function matchupDirection(evaluations, withCritBand) {
	...
	const ceiling = withCritBand ? critCeiling(evaluations) : null;
	return {
		move: best.move,
		max: hp > 0 ? best.damage.max / hp : 0,
		min: hp > 0 ? best.damage.min / hp : 0,
		// The worst case this side can produce, as a share of the same bar:
		// their crit is what a plan has to survive, and our floor is what a
		// plan may rely on. Both travel with the cell so no consumer has to
		// re-derive them from an average.
		...(withCritBand ? {
			critMax: ceiling && hp > 0 ? ceiling.max / hp : (hp > 0 ? best.damage.max / hp : 0),
			critMove: ceiling ? ceiling.move : best.move,
			critKO: !!(ceiling && ceiling.max >= hp),
			critPriority: !!(ceiling && ceiling.priority),
		} : {}),
...
function matchup(options) {
	...
	const grid = theirs.map(enemy => ({
		...
		versus: ours.map(entry => {
			...
			const built = b.buildBattleState({
				aiActive: foe,
				playerActive: player,
				mode: 'Singles',
				field: cellField,
			...
			ai.validateBattleState(built);
			// A cell opens as the fight does: both bodies' entry abilities fire.
			// Until 2026-09-22 the board graded Primal Kyogre without its heavy
			// rain, Drizzle leads without rain and every cell without
			// Intimidate, after the fights themselves had them — so the ranker
			// chose sixes against an easier foe than the one it would meet.
			const state = ai.applyLeadEntries(built, {random: openingStream('cell:' + fight.trainer + ':' +
				foe.species + ':' + player.species)});
			// No crit band for our side: see matchupDirection. Their crit is the
			// plan's problem; ours is upside a plan may not rely on.
			const us = matchupDirection(
				ai.evaluateActions(state, ai.calculateActionFacts, 'player'), false);
			// `skipThem` is for a caller that already KNOWS the other half and
			// is about to substitute it: swapping which moves our Pokemon
			// knows cannot change what the enemy does to it, so recomputing
			// that half produces a number identical to one the caller is
			// holding. It is 43.6% of the cost of a cell.
			//
			// The cell comes back with them and speed as null rather than as
			// a plausible-looking zero, so a caller that forgets to substitute
			// gets an obvious absence instead of a quiet wrong answer.
			if (opts.skipThem) {
				return {
			...
			const them = matchupDirection(
				ai.evaluateActions(state, ai.calculateActionFacts, 'ai'), true);
			return {
				species: player.species,
				level: player.level,
