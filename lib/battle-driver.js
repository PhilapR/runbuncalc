...
/**
 * Whether a won rollout is valued by what it KEPT. OFF until measured.
 *
...
function rolloutOnce(bundle, candidate, seed, dice) {
	...
	return {value: rolloutValue(sim, simReply), parts: rolloutParts(sim, simReply)};
}
...
function searchChoice(bundle, actions, seed, rollouts, dice) {
	...
	if (candidates.length > 1) {
		...
		const scored = candidates.map(candidate => ({candidate, total: 0, runs: 0, wins: 0, removed: 0, oursAlive: 0}));
		...
		// What the search thought of EVERY candidate, not only the one it
		// played: a fight log that shows the choice without the alternatives
		// cannot say whether a line was found or stumbled into.
		return {chosen, dice, scores: scored.map(entry => ({
			...
			// The parts, so a value can be read: playouts won, the share of
			// their HP removed, and how many of ours were left standing.
			wins: entry.wins,
	...
	for (let guard = 0; guard < 150 && actions.length; guard++) {
		...
		if (opts.tape) {
			...
			opts.tape.push({turn: bundle.state.turn, phase: /Choose the next/.test(seen.prompt || '') ? 'replace' : 'choose',
				...
				why: /Choose the next/.test(seen.prompt || '') ? 'forced replacement' : 'search-' + rollouts,
