...
function playDoubles(doc, trainerName, seed, options) {
	...
	const named = entry => {
		...
		const matched = mon && mon.species === entry.species ? mon : null;
...
function rolloutOnce(bundle, candidate, seed, dice) {
	...
	return rolloutValue(sim, simReply);
}
...
function searchChoice(bundle, actions, seed, rollouts, dice) {
	...
	if (candidates.length > 1) {
		...
		const scored = candidates.map(candidate => ({candidate, total: 0, runs: 0}));
		...
		for (const entry of scored) {
			for (let k = 0; k < first; k++) {
				dice += 1;
				entry.total += rolloutOnce(bundle, entry.candidate, seed, dice);
				entry.runs += 1;
			}
		}
		let deciding = scored;
