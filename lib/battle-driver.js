...
function rolloutValue(bundle, reply, meanLead) {
	if (reply && reply.result === 'win') {
		...
		return searchKeepWeight + (1 - searchKeepWeight) * ours.filter(mon => mon.hp.current > 0).length / Math.max(1, ours.length);
	}
