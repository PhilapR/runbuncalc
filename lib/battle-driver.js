...
/**
 * The turn's end — poison, burn, weather, Leech Seed, Curse, Perish Song —
 * with its faints recorded. Every advanceTurn went unrecorded, so a body lost
 * at the end of a turn had no epitaph in singles (the wall view's
 * "unattributed") and was not counted at all in doubles, whose deaths are
 * the faint log (found 2026-09-22).
 */
function advanceRecorded(state, random, events, faints) {
	const after = ai.advanceTurn(state, {random});
	recordFaints(state, after, 'end of turn', null, events, faints);
	return after;
}
...
function recordFaints(before, after, byMove, ofMonId, events, faints) {
	for (const side of ['ai', 'player']) {
		for (const mon of after.sides[side].party) {
			...
			// Whose side the acting body was on. `of` names whoever was ACTING
			// when this body fell, not who dealt the blow: a body of ours that
			// dies to Spikes on the way in is "of" our own outgoing body. A
			// reader that charges `of` to their side needs this to tell.
			const ofSide = !killer ? null :
				after.sides.player.party.some(entry => entry.id === ofMonId) ? 'ours' : 'theirs';
			events.push({text: `${mon.species} fainted!`, side: side === 'player' ? 'ours' : 'theirs'});
			faints.push({
				battleId: mon.id,
				side,
				species: mon.species,
...
function start(doc, trainerName, seed) {
	...
	// The fight's seed decides its opening too (a Trace pick, a speed tie
	// between two leads), from a stream apart from the per-step ones, so the
	// same seed opens the same way and different seeds may not.
	const fightSeed = Number.isInteger(seed) ? seed : Math.floor(Math.random() * 0x7fffffff);
	const built = planner.buildFightState({
		trainer: fight.trainer,
		playerParty: specs,
		profileId: doc.profileId,
		random: streamFor(fightSeed, -1),
	});
...
function act(bundle, chosen) {
	...
	if (bundle.phase === 'replace') {
		...
		if (!finished(state) && phaseOf(state) !== 'replace') state = advanceRecorded(state, rng, events, faints);
		...
		if (shakes === 4) {
			...
			if (!finished(state) && phaseOf(state) !== 'replace') {
				state = advanceRecorded(state, rng, events, faints);
			}
		}
	} else {
	...
	const turn = overrides => {
		...
		for (const entry of ai.orderActions(state, chosen, {random: rng})) {
			...
			try {
				...
				// `side` says whose move it was: the text is species names only, and
				// a reader told the sides apart by guessing from the six, which a
				// mirror (Urshifu against Urshifu at Sidney's double) cannot do.
				// A crit is marked as the singles tape marks it.
				const ours = ((before.sides.player || {}).party || []).some(mon => mon.id === aimed.actorId);
				const crit = (resolution.criticalHitTargets || []).length > 0;
				events.push({turn: state.turn, side: ours ? 'ours' : 'theirs',
					text: findMon(before, aimed.actorId).species + ' used ' + aimed.moveName +
...
function playDoubles(doc, trainerName, seed, options) {
	...
	let state = fillPP(planner.buildFightState({trainer: fight.trainer, playerParty: specs,
		profileId: doc.profileId, doubles: true, random: streamFor(Number.isInteger(seed) ? seed : 1, -1)}).state);
	...
	const named = entry => {
		...
		return {monId: matched ? matched.id : null,
			...
			species: entry.species, by: entry.by, of: entry.of, ofSide: entry.ofSide || null};
	};
	const theirs = machine.faints.filter(entry => entry.side === 'ai');
	// What was left of theirs, as the singles path reports it. A double
	// reported none (`foe: null` in the battery), so every doubles attempt
	// ever recorded had no remainder — and planByPlay, which breaks ties by
	// fewest of theirs left, read 0 for every plan: at a doubles wall where
	// no plan won a planning fight it had no signal at all.
	const foeParty = ((machine.state.sides || {}).ai || {}).party || [];
	const foe = {alive: foeParty.filter(mon => (mon.hp || {}).current > 0).length, of: foeParty.length,
		hpPct: Math.round(100 * foeParty.reduce((sum, mon) => sum + Math.max(0, (mon.hp || {}).current || 0), 0) /
			Math.max(1, foeParty.reduce((sum, mon) => sum + ((mon.hp || {}).max || 0), 0)))};
	return {result: result || 'stuck', turns, deaths: ours.length, engineRefusals: counts.refusals, actions: counts.applied, foe,
...
/**
 * What winning is worth on its own, the rest being the share of ours still
 * standing: 0.5 by the ruling above. Under permadeath a body lost in a win is
 * lost for the run, and at 0.5 a win that costs one of six scores 0.92 against
 * a clean win's 1. Lower pays more for every body kept. Any win must still
 * outrank any loss (at most 0.3), so the weight is refused at or below 0.2.
 */
let searchKeepWeight = 0.5;
function setSearchKeepWeight(weight) {
	if (!(weight > 0.2 && weight <= 1)) throw new Error('--search-keep-weight must be in (0.2, 1], not ' + weight);
	searchKeepWeight = weight;
}
function searchKeepWeightOf() { return searchKeepWeight; }

/**
 * A body's weight in a WON playout, by battle id, set by playSearch for the
 * fight it plays when the caller hands it values (--search-value): a survivor
 * counts 1 plus its value to the road ahead, so a won line that keeps the
 * starter outranks one that keeps a spare. Null: every body counts 1.
 */
let bodyWeights = null;
...
function rolloutValue(bundle, reply, meanLead) {
	if (reply && reply.result === 'win') {
		...
		const weight = mon => (bodyWeights && bodyWeights.has(mon.id) ? bodyWeights.get(mon.id) : 1);
		const whole = ours.reduce((sum, mon) => sum + weight(mon), 0);
		const kept = ours.filter(mon => mon.hp.current > 0).reduce((sum, mon) => sum + weight(mon), 0);
		return searchKeepWeight + (1 - searchKeepWeight) * kept / Math.max(1e-9, whole);
	}
...
function playSearch(doc, trainerName, seed, options) {
	// The party's battle ids are player-1.. in party order: that is how a box id's
	// value reaches the playouts that decide this fight.
	const values = options && options.values;
	bodyWeights = values ? new Map((doc.party || []).map((id, index) => ['player-' + (index + 1), 1 + (values.get(id) || 0)])) : null;
	try {
		return playSearchWith(doc, trainerName, seed, options);
	} finally {
		bodyWeights = null;
	}
}

/** Runs fn with these playout weights (a Map of battle id to weight), then clears them. */
function withBodyWeights(weights, fn) {
	const before = bodyWeights;
	bodyWeights = weights;
	try {
		return fn();
	} finally {
...
function playSearchWith(doc, trainerName, seed, options) {
	const opts = options || {};
