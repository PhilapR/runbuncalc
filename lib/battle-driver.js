...
function legalActions(state) {
	...
	const switches = ai.enumerateSwitchActions(state, 'player').map(action => Object.assign({
		...
		species: findMon(state, action.replacementId).species,
		hp: findMon(state, action.replacementId).hp,
	}, switchPricingOn ? {race: switchRace(state, action.replacementId)} : {}));
...
function act(bundle, chosen) {
	...
	if (bundle.phase === 'replace') {
		...
		if (!finished(state)) state = ai.advanceTurn(state, {random: rng});
		...
		for (const entry of entries) {
			...
			if (!alive(state, action.actorId) || !onField(state, action.actorId)) continue;
			...
			try {
				...
				events.push(refusalEvent(findMon(state, action.actorId).species, error));
			}
			state = settleAiSide(state, events);
			state = clearDeadPlayerPending(state);
 ...
 */
function playDoubles(doc, trainerName, seed) {
	const runtime = require('./run');
	...
	const settle = () => {
		...
		// A pending forced switch nobody can answer is cleared, as in singles.
		if ((state.pendingForcedSwitchIds || []).length) {
			state = Object.assign({}, state, {pendingForcedSwitchIds: state.pendingForcedSwitchIds.filter(id =>
				['ai', 'player'].some(sideId => state.sides[sideId].party.some(mon => mon.id === id) &&
					ai.enumerateForcedSwitchActions(state, sideId).length))});
		}
	};
	const aim = action => {
		if (action.kind !== 'move' || !(action.targetIds || []).length) return action;
		const foeSide = state.sides.ai.party.some(mon => mon.id === action.actorId) ? 'player' : 'ai';
		const standing = id => alive(state, id) && onField(state, id);
		const aimed = action.targetIds.filter(id => state.sides[foeSide].party.some(mon => mon.id === id));
		if (!aimed.length || aimed.every(standing)) return action;
		const partner = state.sides[foeSide].activeIds.find(standing);
		if (!partner) return null;
		const targetIds = action.targetIds.map(id => aimed.includes(id) && !standing(id) ? partner : id)
			.filter((id, index, list) => list.indexOf(id) === index);
		return Object.assign({}, action, {targetIds});
	};
	let turns = 0;
	let result = null;
	for (; turns < 300 && !result; turns++) {
		...
		for (const sideId of ['player', 'ai']) {
			...
			for (const actorId of state.sides[sideId].activeIds) {
				...
				if (mine.length) chosen.push(ai.chooseAction(mine, rng).action);
			}
		}
		for (const entry of ai.orderActions(state, chosen, {random: rng})) {
			...
			try {
				...
				const resolution = ai.deriveMoveResolution(state, aimed, {facts, random: rng});
	...
	const ours = faints.filter(entry => entry.side === 'player');
	return {result: result || 'stuck', turns, deaths: ours.length, engineRefusals: refusals, actions: applied,
		killers: ours.map(entry => ({species: entry.species, by: entry.by, of: entry.of})), events};
}

module.exports = {start, startWild, act, legalActions, view, streamFor, adjudicate, playbook, catchMath, catchOddsAtFullHp, incomingThreat, setPPModel, setSwitchPricing, switchPricing, benchRace, refusalEvent, playDoubles, BALLS};
