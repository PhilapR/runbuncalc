...
/**
 * The rival takes the starter that beats YOURS; picking one fixes their ace.
 *
 * Read from the PROFILE, not written here. This was a hardcoded copy of the
 * Hoenn trio, which is the wrong game: Run & Bun starts the player on the
 * Sinnoh line while leaving the rival's teams built around Hoenn. A second
 * copy of the answer is how the page and the CLI could disagree with the
 * data and with each other.
 */
const STARTER_RIVAL = (() => {
	const starters = (runtime.profileOf ? runtime.profileOf() : null) ||
		require('../profiles').getProfile('run-and-bun').encounters.STARTERS;
	const map = {};
	for (const starter of starters) map[starter.species] = starter.rival;
	return map;
})();
...
function renderEncounters(found) {
	...
	if (found.items && found.items.length) {
		...
		for (const item of found.items) {
			// "order", not "fight #". opensAt counts cumulative enemy Pokemon,
			// so printing it as a fight number sends a reader 51 trainers past
			// the unlock: TM16 opens at order 77, which is Brawly, and Brawly
			// is the 26th fight of 362. renderEncounters has no run to convert
			// with, so it names the scale rather than guessing at the other.
			lines.push(`    ${item.name.padEnd(16)} ${item.kind.padEnd(11)} ` +
				(item.collected ? 'collected' : item.open ? 'UNCOLLECTED — go get it' :
					`opens at order ${item.opensAt}`));
		}
	}
	const byMethod = {};
...
function renderPlan(plan) {
	const lines = [`${plan.trainer} (#${plan.fightNumber != null ? plan.fightNumber : plan.order})` +
...
function renderMatrix(matrix) {
	const lines = [`${matrix.trainer} (#${matrix.fightNumber != null ? matrix.fightNumber : matrix.order})`];
...
function renderRanking(ranking) {
	const lines = [`${ranking.trainer} (#${ranking.fightNumber != null ? ranking.fightNumber : ranking.order}) — ` +
...
function renderAdvice(advice) {
	const lines = [`${advice.trainer} (#${advice.fightNumber != null ? advice.fightNumber : advice.order}) — ${advice.considered} single changes weighed`];
...
const SUBCOMMANDS = {
		...
		if (args.flags.starter) {
			...
			if (args.flags.rival && args.flags.rival !== STARTER_RIVAL[args.flags.starter]) {
				throw new Error(`new: ${args.flags.starter} fixes the rival to ` +
					`${STARTER_RIVAL[args.flags.starter]} — they take the one that beats yours`);
			}
	...
	playbook(state, args) {
		...
		const lines = [
			`${book.trainer} (#${book.fightNumber != null ? book.fightNumber : book.order}) — the playbook, played ${book.odds.rollouts} times`,
	...
	adjudicate(state, args) {
		...
		return {message: [
			`${fight.trainer} (#${runtime.trainerIndexOf(state, fight.order) || fight.order}) — the current party, played ${rollouts} times`,
	...
	graveyard(state) {
		...
		for (const mon of lost) {
			...
			const died = mon.died && mon.died.to ?
				`  killed by ${mon.died.to}${mon.died.move ? `'s ${mon.died.move}` : ''} (#${runtime.trainerIndexOf(state, mon.died.order) || mon.died.order})` :
				'  cause unrecorded';
	...
	split(state, args) {
		...
		if (prep.pickups && prep.pickups.length) {
			...
			for (const item of prep.pickups) {
				// A player counts trainers and the data counts Pokemon, so the
				// conversion happens here, where the run is in scope to do it.
				const at = runtime.trainerIndexOf(state, item.opensAt);
				lines.push(`    ${item.name.padEnd(16)} @ ${item.location.padEnd(22)}` +
					(item.reachable ? 'reachable now' :
						at ? `from fight #${at}` : `from order ${item.opensAt}`));
			}
		}
		return {message: lines.join('\n')};
	},
	...
	scout(state, args) {
		const out = runtime.adviseCatches(state,
			args.positional.length ? args.positional.join(' ') : undefined);
		const lines = [`vs ${out.trainer} (#${out.fightNumber != null ? out.fightNumber : out.order})` +
...
const USAGE = `node play.js <command> [args] [--file run.json]
  ...
  new [--name N] [--no-cap] [--nuzlocke] [--starter Turtwig] [--force]   start a run (game caps on by default)
