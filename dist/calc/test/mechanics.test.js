...
describe('calculator mechanics regressions', () => {
    (0, helper_1.inGens)(2, 2, ({ calculate, Pokemon, Move }) => {
        test('keeps Gen II damage at a minimum of one after the random factor', () => {
            const result = calculate(Pokemon('Magikarp', { level: 5 }), Pokemon('Shuckle', { level: 100 }), Move('Tackle', { overrides: { basePower: 1 } }));
            expect(Math.min(...result.damage)).toBe(1);
        });
    });
    (0, helper_1.inGens)(7, 7, ({ calculate, Pokemon, Move }) => {
    ...
    (0, helper_1.inGens)(7, 9, ({ calculate, Pokemon, Move }) => {
    ...
    (0, helper_1.inGens)(5, 9, ({ calculate, Pokemon, Move, Field }) => {
        ...
        test('gives a combined Pledge move forced STAB', () => {
            ...
            expect(Math.max(...combined.damage)).toBeGreaterThan(Math.max(...unboosted.damage));
        });
    });
    (0, helper_1.inGens)(8, 9, ({ calculate, Pokemon, Move }) => {
