...
describe('Run & Bun fork behavior', () => {
    (0, helper_1.inGen)(8, ({ calculate, Pokemon, Move, Field }) => {
        ...
        test('applies the fork terrain damage boost in modern generations', () => {
            const result = calculate(Pokemon('Mewtwo', { nature: 'Timid', evs: { spa: 252 }, boosts: { spa: 2 } }), Pokemon('Milotic', {
                item: 'Flame Orb',
                nature: 'Bold',
        ...
        test('keeps Gale Wings priority when the attacker is not at full HP', () => {
            const result = calculate(Pokemon('Mew', { ability: 'Gale Wings', originalCurHP: 1 }), Pokemon('Vulpix'), Move('Air Slash'));
            expect(result.move.priority).toBe(1);
        });
        test('models the fork Disguise first-hit protection when explicitly intact', () => {
            const result = calculate(Pokemon('Mew'), Pokemon('Mimikyu', { ability: 'Disguise', disguiseBroken: false }), Move('Tackle'));
        ...
        test('does not create a Parental Bond child hit while the ability is off', () => {
            const active = calculate(Pokemon('Kangaskhan', { ability: 'Parental Bond', abilityOn: true }), Pokemon('Vulpix'), Move('Tackle'));
            const suppressed = calculate(Pokemon('Kangaskhan', { ability: 'Parental Bond', abilityOn: false }), Pokemon('Vulpix'), Move('Tackle'));
            expect(active.range()[0]).toBeGreaterThan(suppressed.range()[0]);
            expect(suppressed.rawDesc.attackerAbility).toBeUndefined();
        });
    });
    (0, helper_1.inGen)(9, ({ Pokemon }) => {
        test('uses the fork Azumarill base Attack data', () => {
            expect(Pokemon('Azumarill').species.baseStats.atk).toBe(65);
        });
    });
    (0, helper_1.inGens)(3, 8, ({ calculate, Pokemon, Move }) => {
        test('honors explicit grounding overrides for Ground-type damage', () => {
            const airborne = calculate(Pokemon('Pikachu'), Pokemon('Rattata', { isGrounded: false }), Move('Earthquake'));
            const grounded = calculate(Pokemon('Pikachu'), Pokemon('Rattata', { isGrounded: true }), Move('Earthquake'));
        ...
        test('uses the fork critical-hit prevention for Magma Armor', () => {
            const result = calculate(Pokemon('Mew'), Pokemon('Vulpix', { ability: 'Magma Armor' }), Move('Slash', { isCrit: true }));
            expect(result.rawDesc.isCritical).toBeFalsy();
        });
        test('uses the fork 1.5x critical-hit damage multiplier', () => {
            const critical = calculate(Pokemon('Mew'), Pokemon('Vulpix'), Move('Slash', { isCrit: true }));
    ...
    (0, helper_1.inGens)(1, 2, ({ calculate, Pokemon, Move }) => {
        test('uses the fork 1.5x critical-hit damage multiplier in legacy generations', () => {
            const critical = calculate(Pokemon('Mew'), Pokemon('Vulpix'), Move('Tackle', { isCrit: true }));
    ...
    (0, helper_1.inGens)(4, 8, ({ calculate, Pokemon, Move, Field }) => {
        test('Lucky Chant prevents forced critical hits', () => {
            const luckyChant = calculate(Pokemon('Mew'), Pokemon('Vulpix'), Move('Slash', { isCrit: true }), Field({ defenderSide: { isLuckyChant: true } }));
            const normal = calculate(Pokemon('Mew'), Pokemon('Vulpix'), Move('Slash'));
            expect(luckyChant.rawDesc.isCritical).toBeFalsy();
            expect(luckyChant.range()).toEqual(normal.range());
        });
    });
    (0, helper_1.inGens)(5, 8, ({ calculate, Pokemon, Move }) => {
        test('allows Ground damage against a landed Flying target', () => {
            const landedFlying = calculate(Pokemon('Pikachu'), Pokemon('Pidgey', { isGrounded: true }), Move('Earthquake'));
            expect(landedFlying.range()).not.toEqual([0, 0]);
        });
    });
    (0, helper_1.inGens)(5, 9, ({ calculate, Pokemon, Move }) => {
        test('uses the fork Iron Ball grounding behavior', () => {
            const result = calculate(Pokemon('Vibrava'), Pokemon('Zapdos', { item: 'Iron Ball' }), Move('Earthquake'));
            expect(result.range()).toEqual([186, 218]);
        });
    });
    (0, helper_1.inGens)(3, 7, ({ calculate, Pokemon, Move, Field }) => {
        test('applies Water Sport and Mud Sport field power reductions', () => {
            ...
            const fireReduced = calculate(Pokemon('Mew'), Pokemon('Rattata'), Move('Flamethrower'), Field({ isWaterSport: true }));
            const electric = calculate(Pokemon('Mew'), Pokemon('Rattata'), Move('Thunderbolt'));
            const electricReduced = calculate(Pokemon('Mew'), Pokemon('Rattata'), Move('Thunderbolt'), Field({ isMudSport: true }));
    ...
    (0, helper_1.inGen)(8, ({ calculate, Pokemon, Move, Field }) => {
        test('does not apply unavailable Water Sport or Mud Sport in Gen 8', () => {
            const fire = calculate(Pokemon('Mew'), Pokemon('Rattata'), Move('Flamethrower'));
            const fireWithWaterSport = calculate(Pokemon('Mew'), Pokemon('Rattata'), Move('Flamethrower'), Field({ isWaterSport: true }));
            expect(fireWithWaterSport.range()).toEqual(fire.range());
        });
    });
    (0, helper_1.inGens)(6, 7, ({ calculate, Pokemon, Move, Field }) => {
        test('Ion Deluge converts Normal moves to Electric', () => {
            const normal = calculate(Pokemon('Mew'), Pokemon('Pidgey'), Move('Tackle'));
            const ion = calculate(Pokemon('Mew'), Pokemon('Pidgey'), Move('Tackle'), Field({ isIonDeluge: true }));
            expect(ion.rawDesc.moveType).toBe('Electric');
            expect(ion.range()[0]).toBeGreaterThan(normal.range()[1]);
        });
    });
    (0, helper_1.inGen)(5, ({ calculate, Pokemon, Move, Field }) => {
